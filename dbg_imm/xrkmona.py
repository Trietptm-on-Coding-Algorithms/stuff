...
try:
    ...
    try:
        ...
        # exit(1)
        # TODO: remove non-debugger utils to other files
        pass
...
def getAddyArg(argaddy):
    """
        Tries to extract an address from a specified argument addresses and values will be considered hex
...
def isHexValue(str_):
    """
        Check if a str_ is a hex value / consists of hex chars only (and - )

        Arguments:
        str_ - the str_ to check

        Return:
        Boolean - True if the address str_ only contains hex bytes or - sign
    """
    str_ = str_.replace("\\x", "")
...
def bytesInRange(address, range):
    ...
    if arch == 32:
        ...
        if not (byte1 == 0 or byte1 in range):
            return False
        elif byte2 not in range:
            return False
        elif byte3 not in range:
            return False
        elif byte4 not in range:
            return False
    ...
    if arch == 64:
        ...
        if not (byte1 == 0 or byte1 in range):
            ...
            return False
        elif byte5 not in range:
            return False
        elif byte6 not in range:
            return False
        elif byte7 not in range:
            return False
        elif byte8 not in range:
            return False
...
def meetsAccessLevel(page, accessLevel):
    ...
    if "-X" in accessLevel:
        if "EXECUTE" in pageAccess:
            return False
    if "R" in accessLevel:
        if "READ" not in pageAccess:
            return False
    if "W" in accessLevel:
        if "WRITE" not in pageAccess:
            return False
    if "X" in accessLevel:
        if "EXECUTE" not in pageAccess:
            return False
...
class MnDeferredHook(LogBpHook):
    ...
    def run(self, regs):
        ...
        if self.targetptr.find("+") > -1:
            ...
            if themodule is not None and len(ptrparts) > 1:
...
class MnHeap:
    ...
    def getFreeListInUseBitmap(self):
        global FreeListBitmap
        if self.heapbase not in FreeListBitmap:
            FreeListBitmapHeap = []
            cnt = 0
            while cnt < 4:
    ...
    def getVirtualAllocdBlocks(self):
        ...
        if self.heapbase not in VACache:
            try:
...
class MnSegment:
    ...
    def getChunks(self):
        ...
        while not allchunksfound:
            ...
            if thischunk not in allchunks and thissize > 0:
                ...
                try:
                    ...
                    if "mshtml!" in outputlower and ieversion > 7:
                        ...
                        try:
                            ...
                            pass
                        if refcounter is not None and attributeptr is not None:
    ...
    if heapbase in segmentlistCache:
        ...
        try:
            if win7mode:
                ...
                if firstsegbase not in segmentinfo:
                ...
                while not allsegmentsfound:
                    ...
                    if thisbase > 0 and thisbase not in segmentinfo:
...
def searchInRange(sequences, start=0, end=TOP_USERLAND, criteria=[]):
    ...
    if "accesslevel" not in criteria:
    ...
    if (ptr_to_get < 0) or (ptr_to_get > 0 and ptr_counter < ptr_to_get):
        ...
        for a in dbg.MemoryPages.keys():
            ...
            if (ptr_to_get < 0) or (ptr_to_get > 0 and ptr_counter < ptr_to_get):
                ...
                if ((("startswithnull" in criteria and criteria["startswithnull"])) and (start_fb != "00" or end_fb != "00")):
                    if not silent:
                ...
                if len(modulenamewithout) <= len(thismodkey):
                    ...
                    if modulename[0] == "*":
                        if modulenamewithout == thismodkey[len(thismodkey) - len(modulenamewithout):len(thismodkey)]:
                            if thismod.moduleKey not in modulestoquery and not thismod.isExcluded:
                    ...
                    if modulename[len(modulename) - 1] == "*":
                        if (modulenamewithout == thismodkey[0:len(modulenamewithout)] and not thismod.isExcluded):
                            if thismod.moduleKey not in modulestoquery:
                                modulestoquery.append(thismod.moduleKey)
                    # contains ?
                ...
                for fn in allfuncs:
                    for fname in fnames:
                        if allfuncs[fn].lower().find(fname.lower()) > -1:
                            ...
                            if fname not in offsetpointers:
                    ...
                    if pmodname.lower() in offsets:
                        ...
                        for interestingfunc in offsets[pmodname.lower()]:
                            if interestingfunc in offsetpointers:
                                ...
                                if thetype not in ropfuncoffsets:
...
def findROPGADGETS(modulecriteria={}, criteria={}, endings=[], maxoffset=40, depth=5, split=False, pivotdistance=0, fast=False, mode="all"):
            ...
            for endingtypeptr in all_opcodes[endingtype]:
                ...
                if not usefiles:
                    ...
                    while startptr <= endingtypeptr and startptr != 0x0:
                        ...
                        thisopcodebytes = ""
                        chainptr = startptr
                        if isGoodGadgetPtr(startptr, criteria) and startptr not in ropgadgets and startptr not in interestinggadgets:
                            invalidinstr = False
                            ...
                            if endingtypeptr == chainptr and startptr != chainptr and not invalidinstr:
                                ...
                                if isInterestingGadget(fullchain):
                                    ...
                                    if stackpivotdistance > 0:
                                        # safeseh or not ?
                                        if issafeseh:
                                            if stackpivotdistance not in stackpivots_safeseh:
                                            ...
                                            if stackpivotdistance not in stackpivots:
    ...
    try:
        with open(thislog, "a") as fh:
            arrtowrite = ""
            # returns sorted keys as an array
            stackpivots_index = sorted(stackpivots)
        ...
        if len(all_opcodes[endingtype]) > 0:
            for endingtypeptr in all_opcodes[endingtype]:
                ...
                while startptr <= endingtypeptr and startptr != 0x0:
                    ...
                    thisopcodebytes = ""
                    chainptr = startptr
                    if isGoodGadgetPtr(startptr, criteria) and startptr not in jopgadgets and startptr not in interestinggadgets:
                        # new pointer
                        invalidinstr = False
                        while chainptr < endingtypeptr and not invalidinstr:
                            thisopcode = dbg.disasm(chainptr)
...
def findFILECOMPARISON(modulecriteria={}, criteria={}, allfiles=[], tomatch="", checkstrict=True, rangeval=0, fast=False):
    ...
    for thisfile in allfiles:
        ...
        for thisLine in content:
            ...
            instr = instr.replace('\n', '').replace('\r', '').strip(":")
            if refpointer != -1 and refpointer not in filedata:
    ...
    if rangeval == 0:
        ...
        for refptr in reflist:
            ...
            while start_range <= end_range:
                if start_range not in allrefptr:
...
def createPattern(size, args={}):
    ...
    if not silent:
        if "extended" not in args and size > 20280 and (len(char1) <= 26 or len(char2) <= 26 or len(char3) <= 10):
...
def findPatternWild(modulecriteria, criteria, pattern, base, top, patterntype):
    ...
    if len(modulecriteria) > 0:
        ...
        if "modules" not in modulecriteria:
            ...
            while thislevel <= level:
                ...
                for ranges in p2prangestosearch:
                    ...
                    for ptrtype in pointers:
                        if ptrtype not in remainingpointers:
                    ...
                    if "pop " + fromreg in suggestions and "add value to " + fromreg in suggestions:
                        ...
                        for addinstr in suggestions["add value to " + fromreg]:
                            if not gadgetfound:
                                ...
                                totalvalue = 0
                                # adget might contain multiple add/sub instructions
...
def main(args):
    ...
    try:
        ...
        def procConfig(args):
            ...
            showerror = False
            if "set" not in args and "get" not in args and "add" not in args:
                showerror = True
        ...
        def procBp(args):
            ...
            if thistype not in valid_types:
        ...
        def procSuggest(args):
            ...
            if exploittype not in exploittypes:
        ...
        def procHeap(args):
            ...
            if len(allheaps) > 0:
                ...
                if searchtype == "" and "stat" not in args:
                                ...
                                for block in sortedblocks:
                                                        ...
                                                        if objconstr not in vtableCache:
                                                            ...
                                                            if "HeapAlloc" not in objoutput:
                                                                ...
                                                                for opline in oplines:
                                                                    if oppat in opline and "del" not in opline:
                                    ...
                                    for optr in allobjects:
                                        ...
                                        if thisvname == prevvname and (optr - prevptr) <= minvtabledistance:
                                            if optr not in objectstodelete:
                                                objectstodelete.append(optr)
                                        else:
                                            prevptr = optr
                                            prevvname = thisvname
            ...
            if findaddy > 0:
                for thispage in orderedpages:
                    ...
                    if pagefound and previouspage > 0:
                        if previouspage not in toshow:
                            toshow.append(previouspage)
                        if thispage not in toshow:
                            # nextpage
                            toshow.append(thispage)
                        break
        ...
        def procEnc(args):
            ...
            if encodertype not in validencoders:
                encodertyperror = True
        ...
        def procString(args):
            ...
            stringtowrite = ""
            # read or write ?
            if "r" not in args and "w" not in args:
            ...
            stringerror = False
            if "w" in args and "s" not in args:
                stringerror = True
            if "s" in args:
                if type(args["s"]).__name__.lower() != "bool":
        ...
        def procUnicodeAlign(args):
            ...
            address = 0
            alignresults = {}
            # we will put ebp into the buffer register
            bufferRegister = "eax"
            timeToRun = 15
        ...
        def prepareAlignment(leaks, address, bufferRegister, timeToRun, registers):
            ...
            prefix = ""
            postfix = ""
            # Length of the prefix+postfix instructions in after-unicode-conversion bytes
            additionalLength = 0
            # \x6d --> \x00\x6d\x00
            ...
            if not leadingZero:
                ...
                # something 1 byte, doesn't matter what
                prefix += "push ebp; "
                leadingZero = True
                additionalLength += 2
        ...
        def generateAlignment(alignment_code_loc, bufferRegister, registers, timeToRun, prefix, postfix, additionalLength):
            ...
            def metasm(inputInstr):
                ...
                ass_operation = {
                    'add [ebp],ch': '\\x00\x6d\\x00', 'pop ebp': ']', 'pop edx': 'Z', 'pop ecx': 'Y', 'push ecx': 'Q',
                ...
                for example_instr, example_op in [
                    ("mov eax,0x41004300", "\\xb8\\x00\\x43\\x00\\x41"),
        ...
        def procDiffHeap(args):
            ...
            if "before" not in args and "after" not in args and "diff" not in args:
                            ...
                            for thisaddy in p:
                                if showfuncposition:
                                    if previnstruction == "" or previnstruction.startswith("RET") or previnstruction.startswith("J") or previnstruction.startswith("CALL"):
                                        if thisaddy not in funcnamecache:
                                ...
                                for c in cregsb:
                                    ...
                                    combins.append("+%s" % c)
                                    for comb in combins:
                                        if comb in instruction and c not in clist:
                                            clist.append(c)
                                ...
                                for c in cregscb:
                                    ...
                                    combins.append("+%s" % c)
                                    for comb in combins:
                                        if comb in instruction and c not in clistc:
                                            clistc.append(c)
