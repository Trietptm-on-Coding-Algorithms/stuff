...
__desc__ = "modified version of pefile by Ero Carrera"
...
# It's there to prevent loading massive amounts of data from memory mapped files.
# Strings longer than 1MB should be rather rare.
# 2^20
...
def get_sublang_name_for_lang(lang_value, sublang_value):
    ...
    for sublang_name in SUBLANG.get(sublang_value, list()):
        # if the main language is a substring of sublang's name, then return that
...
def retrieve_flags(flag_dict, flag_filter):
    """
        ...
        Will return a list of (flag, value) for all flags in "flag_dict" matching the filter "flag_filter".
    """
...
# These come from the great article[1] which contains great insights on working with unicode in both Python 2 and 3.
...
class UnicodeStringWrapperPostProcessor(object):
    """
        This class attempts to help the process of identifying strings that might be plain Unicode or Pascal.
        A list of strings will be wrapped on it with the hope the overlappings will help make the decision about their type.
    """
    ...
    def invalidate(self):
        ...
        """
        # lyc. this is not called, so, whatever...
        # self = None
        pass
...
class Structure(object):
    ...
    def __set_format__(self, format):
        ...
        for elm in format:
            if ',' in elm:
                ...
                # Some PE header structures have unions on them, so a certain value might have different names,
                # so each key has a list of all the possible members referring to the data.
    ...
    def __pack__(self):
        ...
        for i in range(len(self.__unpacked_data_elms__)):
            ...
            for key in self.__keys__[i]:
                ...
                # In the case of Unions, when the first changed value is picked the loop is exited
                if new_val != old_val:
                    break
    ...
    def dump(self, indentation=0):
        ...
        # Refer to the __set_format__ method for an explanation of the following construct.
...
class SectionStructure(Structure):
    ...
    def get_data(self, start=None, length=None):
        """
            ...
            Allows to query data from the section by passing the addresses where the PE file would be loaded by default.
            It is then possible to retrieve code and data by its real addresses as it would be if loaded.
        ...
        # PointerToRawData is not adjusted here as we might want to read any possible extra bytes that might get cut off by aligning the start (and hence cutting something off the end)
    ...
    def contains_rva(self, rva):
        ...
        """
        # Check if the SizeOfRawData is realistic.
        # If it's bigger than the size of the whole PE file minus the start address of the section it could be either truncated or the SizeOfRawData contain a misleading value.
        ...
        # Check whether there's any section after the current one that starts before the calculated end for the current one,
        # if so, cut the current section's size to fit in the range up to where the next section starts.
...
class ImportData(DataContainer):
    """
        ...
        bound:      If the symbol is bound, this contains the address.
    """

    def __setattr__(self, name, val):
        # If the instance doesn't yet have an ordinal attribute it's not fully initialized so can't do any of the following
...
class ExportData(DataContainer):
    """
        ...
        name:       name of the symbol (None if the symbol is exported by ordinal only)
        forwarder:  if the symbol is forwarded it will contain the name of the target symbol, None otherwise.
    """
...
class ResourceDirEntryData(DataContainer):
    """
        ...
        name:       If the resource is identified by name this attribute will contain the name string. None otherwise.
                    If identified by id, the id is available at 'struct.Id'
        id:         the id, also in struct.Id
        directory:  If this entry has a lower level directory this attribute will point to the ResourceDirData instance representing it.
        data:       If this entry has no further lower directories and points to the actual resource data, this attribute will reference the correspondin ResourceDataEntryData instance.
...
class RelocationData(DataContainer):
    """
        ...
        type:       Type of relocation
                    The type string is can be obtained by RELOCATION_TYPE[type]
        rva:        RVA of the relocation
    """
    def __setattr__(self, name, val):
        # If the instance doesn't yet have a struct attribute it's not fully initialized so can't do any of the following
...
class BoundImportDescData(DataContainer):
    """
        ...
        This directory entry will provide with information on the DLLs this PE files has been bound to (if bound at all).
        The structure will contain the name and timestamp of the DLL at the time of binding so that the loader can know whether it differs from the one currently present in the system and must, therefore, re-bind the PE's imports.

        struct:     IMAGE_BOUND_IMPORT_DESCRIPTOR structure
        name:       DLL name
        entries:    list of entries (BoundImportRefData instances) the entries will exist if this DLL has forwarded symbols. If so, the destination DLL will have an entry in this list.
    """
...
# Python 2.x
    ...
    allowed_filename = b(string.lowercase + string.uppercase + string.digits + b"!#$%&'()-@^_`{}~+,.;=[]")
...
# Check if a imported name uses the valid accepted characters expected in mangled function names. If the symbol's characters don't fall within this charset
# we will assume the name is invalid
if PY3:
    allowed_function_name = b(string.ascii_lowercase + string.ascii_uppercase + string.digits + '_?@$()<>')
else:
    allowed_function_name = b(string.lowercase + string.uppercase + string.digits + b'_?@$()<>')
...
class PE(object):
    """
        ...
        to process and an optional argument 'v_fast_load' (False by default)
        ...
        The "v_fast_load" can be set to a default by setting its value in the
        module itself by means, for instance, of a "pefile.v_fast_load = True".
    ...
    def __init__(self, name=None, data=None, fast_load=None):
        ...
        # That will allow for an easy iteration through the list in order to save the modifications made
        self.__structures__ = []
        self.__from_file = None

        if not fast_load:
            fast_load = globals()['v_fast_load']
        try:
    ...
    def __parse__(self, fname, data, v_fast_load):
        """
            parse a pe file

            Loads a PE file, parsing all its structures and making them available through the instance's attributes.

            @param: fname: STRING: file name
            @param: data: BUF
            @param: v_fast_load:
        """
        # 1. init self.__data__
        if fname:
            stat = os.stat(fname)
            if stat.st_size == 0:
        ...
        self.NT_HEADERS = self.__unpack_data__(
            ...
            self.__data__[nt_headers_offset:nt_headers_offset + 8], file_offset=nt_headers_offset)

        # We better check the signature right here, before the file screws around with sections:
        ...
        # According to solardesigner's findings for his Tiny PE project, the optional header does not need fields beyond "Subsystem" in order to be loadable by the Windows loader (given that zeros are acceptable values and the header is loaded in a zeroed memory page)
        # If trying to parse a full Optional Header fails we try to parse it again with some 0 padding
        MINIMUM_VALID_OPTIONAL_HEADER_RAW_SIZE = 69

        if (self.OPTIONAL_HEADER is None and len(self.__data__[optional_header_offset:optional_header_offset + 0x200]) >= MINIMUM_VALID_OPTIONAL_HEADER_RAW_SIZE):
        ...
        # Check the Magic in the OPTIONAL_HEADER and set the PE file type accordingly
        if self.OPTIONAL_HEADER is not None:
            ...
            if self.OPTIONAL_HEADER.Magic == OPTIONAL_HEADER_MAGIC_PE:
                ...
                self.OPTIONAL_HEADER = self.__unpack_data__(self.__IMAGE_OPTIONAL_HEADER64_format__, self.__data__[optional_header_offset:optional_header_offset + 0x200], file_offset=optional_header_offset)

                # Again, as explained above, we try to parse a reduced form of the Optional Header which is still valid despite not including all structure members
        ...
        # The NumberOfRvaAndSizes is sanitized to stay within reasonable limits so can be casted to an int
        ...
        for i in range(int(0x7fffffff & self.OPTIONAL_HEADER.NumberOfRvaAndSizes)):
            ...
            # If the offset goes outside the optional header, the loop is broken, regardless of how many directories NumberOfRvaAndSizes says there are
            # We assume a normally sized optional header, hence that we do a sizeof() instead of reading SizeOfOptionalHeader.
            # Then we add a default number of directories times their size, if we go beyond that, we assume the number of directories is wrong and stop processing
            if offset >= (optional_header_offset + self.OPTIONAL_HEADER.sizeof() + 8 * 16):
                break

        offset = self.parse_sections(sections_offset)

        # There could be a problem if there are no raw data sections greater than 0
        # fc91013eb72529da005110a3403541b6 example
        # Should this throw an exception in the minimum header offset can't be found?
    ...
    def parse_sections(self, offset):
        """
            ...
            Its attributes will contain all the section information plus "data" a buffer containing the section's data.

            The "Characteristics" member will be processed and attributes representing the section characteristics (with the 'IMAGE_SCN_' string trimmed from the constant's names) will be added to the section instance.
        ...
        for i in range(self.FILE_HEADER.NumberOfSections):
            ...
            if count_zeroes(section_data) == section.sizeof():
                self.__warnings.append('Invalid section {0}. Contents are null-bytes.'.format(i))
                break
            if len(section_data) == 0:
                self.__warnings.append('Invalid section {0}. No data in the file (is this corkami\'s virtsectblXP?).'.format(i))
                break
            ...
            if section.SizeOfRawData + section.PointerToRawData > len(self.__data__):
                ...
                self.__warnings.append('Error parsing section {0}. SizeOfRawData is larger than file.'.format(i))

            if self.adjust_FileAlignment(section.PointerToRawData, self.OPTIONAL_HEADER.FileAlignment) > len(self.__data__):
                simultaneous_errors += 1
                self.__warnings.append('Error parsing section {0}. PointerToRawData points beyond the end of the file.'.format(i))

            if section.Misc_VirtualSize > 0x10000000:
                simultaneous_errors += 1
                self.__warnings.append('Suspicious value found parsing section {0}. VirtualSize is extremely large > 256MiB.'.format(i))
            ...
            if self.adjust_SectionAlignment(section.VirtualAddress, self.OPTIONAL_HEADER.SectionAlignment, self.OPTIONAL_HEADER.FileAlignment) > 0x10000000:
                simultaneous_errors += 1
                self.__warnings.append('Suspicious value found parsing section {0}. VirtualAddress is beyond 0x10000000.'.format(i))

            if (self.OPTIONAL_HEADER.FileAlignment != 0 and (section.PointerToRawData % self.OPTIONAL_HEADER.FileAlignment) != 0):
                simultaneous_errors += 1
                self.__warnings.append(('Error parsing section {0}. PointerToRawData should normally be a multiple of FileAlignment, this might imply the file is trying to confuse tools which parse this incorrectly.').format(i))
            ...
            if (section.__dict__.get('IMAGE_SCN_MEM_WRITE', False) and section.__dict__.get('IMAGE_SCN_MEM_EXECUTE', False)):
                ...
                if section.Name == 'PAGE' and self.is_driver():
                    # Drivers can have a PAGE section with those flags set without implying that it is malicious
                    pass
                else:
        ...
        # Sort the sections by their VirtualAddress and add a field to each of them with the VirtualAddress of the next section.
        # This will allow to check for potentially overlapping sections in badly constructed PEs.
    ...
    def parse_data_directories(self, directories=None, forwarded_exports_only=False, import_dllnames_only=False):
        """
            ...
            If the optional argument 'directories' is given, only the directories at the specified indexes will be parsed.
            Such functionality allows parsing of areas of interest without the burden of having to parse all others.
            ...
            If `forwarded_exports_only` is True, the IMAGE_DIRECTORY_ENTRY_EXPORT attribute will only contain exports that are forwarded to another DLL.
            If `import_dllnames_only` is True, symbols will not be parsed from the import table and the entries in the IMAGE_DIRECTORY_ENTRY_IMPORT attribute will not have a `symbols` attribute.
        """
        directory_parsing = (('IMAGE_DIRECTORY_ENTRY_IMPORT', self.parse_directory_import),
                             ('IMAGE_DIRECTORY_ENTRY_EXPORT', self.parse_directory_export),
                             ('IMAGE_DIRECTORY_ENTRY_RESOURCE', self.parse_directory_resources),
                             ('IMAGE_DIRECTORY_ENTRY_DEBUG', self.parse_directory_debug),
                             ('IMAGE_DIRECTORY_ENTRY_BASERELOC', self.parse_directory_relocations),
                             ('IMAGE_DIRECTORY_ENTRY_TLS', self.parse_directory_tls),
                             ('IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG', self.parse_directory_load_config),
                             ('IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT', self.parse_directory_delay_import),
        ...
        for entry in directory_parsing:
            ...
            # Only process all the directories if no individual ones have been chosen
    ...
    def parse_directory_import_get_import_table(self, rva, max_length=None):
        """
            get import table
        """
        table = []
        ...
        if self.PE_TYPE == OPTIONAL_HEADER_MAGIC_PE:
            ...
            # Some PEs may have an invalid value in the Magic field of the Optional Header.
            # Just in case the remaining file is parseable let's pretend it's a 32bit PE32 by default.
            ordinal_flag = IMAGE_ORDINAL_FLAG
            format = self.__IMAGE_THUNK_DATA_format__
        ...
        while True and rva:
            ...
            # if we see too many times the same entry we assume it could be a table containing bogus data (with malicious intent or otherwise)
            if repeated_address >= MAX_REPEATED_ADDRESSES:
                return []

            # if the addresses point somewhere but the difference between the highest and lowest address is larger than MAX_ADDRESS_SPREAD we assume a bogus table as the addresses should be contained within a module
            ...
            # Check if the AddressOfData lies within the range of RVAs that it's being scanned, abort if that is the case, as it is very unlikely to be legitimate data.
            ...
            if thunk_data and thunk_data.AddressOfData:
                ...
                if thunk_data.AddressOfData & ordinal_flag:
                    # but its value is beyond 2^16, we will assume it's a corrupted and ignore it altogether
                    if thunk_data.AddressOfData & 0x7fffffff > 0xffff:
                        return []
                # and if it looks like it should be an RVA
                else:
                    # keep track of the RVAs seen and store them to study their properties.
                    # When certain non-standard features are detected the parsing will be aborted
    ...
    def parse_directory_import_or_delay_item(self, original_first_thunk, first_thunk, forwarder_chain, max_length=None):
        """
            Parse the imported symbols.

            It will fill a list, which will be available as the dictionary attribute "imports".
            Its keys will be the DLL names and the values all the symbols imported from that object.
        """

        imported_symbols = []

        # The following has been commented as a PE does not need to have the import data necessarily within a section, it can keep it in gaps between sections or overlapping other data.
        ...
        ilt = self.parse_directory_import_get_import_table(original_first_thunk, max_length)
        # Import Address Table. May have identical content to ILT if PE file is not bounded, Will contain the address of the imported symbols once the binary is loaded or if it is already bound.
        iat = self.parse_directory_import_get_import_table(first_thunk, max_length)

        # Would crash if IAT or ILT had None type
        if (not iat or len(iat) == 0) and (not ilt or len(ilt) == 0):
            self.__warnings.append('Damaged Import Table information. ILT and/or IAT appear to be broken. OriginalFirstThunk: 0x{0:x} FirstThunk: 0x{1:x}'.format(original_first_thunk, first_thunk))
            return []
        ...
        if self.PE_TYPE == OPTIONAL_HEADER_MAGIC_PE:
            ...
            # Some PEs may have an invalid value in the Magic field of the Optional Header.
            # Just in case the remaining file is parseable
        ...
        for idx in range(len(table)):
            ...
            #
            # has an invalid table built in a way that it's parseable but contains invalid entries that lead pefile to take extremely long amounts of time to parse.
            # It also leads to extreme memory consumption.
            # To prevent similar cases, if invalid entries are found in the middle of a table the parsing will be aborted
            if imp_ord is None and imp_name is None:
                raise PEFormatError('Invalid entries, aborting parsing.')

            # Some PEs appear to interleave valid and invalid imports.
            # Instead of aborting the parsing altogether we will simply skip the invalid entries.
    ...
    def parse_directory_import(self, rva, size, dllnames_only=False):
        """
        ...
        while True:
            try:
                # If the RVA is invalid all would blow up. Some EXEs seem to be specially nasty and have an invalid RVA.
            ...
            # If the array of thunk's is somewhere earlier than the import descriptor we can set a maximum length for the array.
            ...
            if not dllnames_only:
                try:
                    import_data = self.parse_directory_import_or_delay_item(import_desc.OriginalFirstThunk, import_desc.FirstThunk, import_desc.ForwarderChain, max_length=max_len)
                except PEFormatError as e:
                    self.__warnings.append('Error parsing the import directory. Invalid Import data at RVA: 0x{0:x} ({1})'.format(rva, e.value))

                if error_count > 5:
                    self.__warnings.append('Too may errors parsing the import directory. Invalid import data at RVA: 0x{0:x}'.format(rva))
                    break
            ...
            for imp_dll in import_descs:
                for symbol in imp_dll.imports:
                    for suspicious_symbol in suspicious_imports:
                        if symbol and symbol.name and symbol.name.startswith(b(suspicious_symbol)):
    ...
    def parse_directory_delay_import(self, rva, size):
        """
        ...
        while True:
            try:
                # If the RVA is invalid all would blow up. Some PEs seem to be specially nasty and have an invalid RVA.
            ...
            import_data = []
            try:
                import_data = self.parse_directory_import_or_delay_item(import_desc.pINT, import_desc.pIAT, None, max_length=max_len)
            except PEFormatError as e:
                self.__warnings.append('Error parsing the Delay import directory. Invalid import data at RVA: 0x{0:x} ({1})'.format(rva, e.value))

            if error_count > 5:
                self.__warnings.append('Too may errors parsing the Delay import directory. Invalid import data at RVA: 0x{0:x}'.format(rva))
                break
    ...
    def parse_directory_export(self, rva, size, forwarded_only=False):
        """
        ...
        # We keep track of the bytes left in the file and use it to set a upper bound in the number of items that can be read from the different arrays.
        ...
        if not section:
            self.__warnings.append('RVA AddressOfNames in the export directory points to an invalid address: %x' % export_dir.AddressOfNames)
            return
        else:
        ...
        for i in range(min(export_dir.NumberOfNames, int(safety_boundary / 4))):
            ...
            if symbol_ordinal is not None and symbol_ordinal * 4 < len(address_of_functions):
                ...
                # Corrupt? a bad pointer... we assume it's all useless, no exports
                return None
            ...
            # it will point to a string with the forwarded symbol's string instead of pointing the the function start address.
        ...
        if not section:
            self.__warnings.append('RVA AddressOfFunctions in the export directory points to an invalid address: %x' % export_dir.AddressOfFunctions)
            return
        else:
    ...
    def parse_directory_resources_item(self, rva):
        """
    ...
    def parse_directory_resources_entry(self, rva):
        """
    ...
    def parse_directory_resources_version_info(self, version_struct):
        """
            ...
            StringFileInfo structures will have a list as an attribute named 'StringTable' containing all the StringTable structures.
            Each of those structures contains a dictionary 'entries' with all the key / value version information string pairs.

            VarFileInfo structures will have a list as an attribute named 'Var' containing all Var structures.
            Each Var structure will have a dictionary as an attribute named 'entry' which will contain the name and value of the Var.
        """
        ...
        # These should return 'ascii' decoded data. For the case when it's garbled data the ascii string will retain the byte values while encoding it to something else may yield values that don't match the file's contents.
        try:
            ...
            if stringfileinfo_string and stringfileinfo_string.startswith(b'StringFileInfo'):
                ...
                if stringfileinfo_struct.Type in (0, 1) and stringfileinfo_struct.ValueLength == 0:
                    ...
                    while True:
                        ...
                        # Check if the entry is crafted in a way that would lead to an infinite loop and break if so.
    ...
    def parse_directory_resources(self, rva, size=0, base_rva=None, level=0, dirs=None):
        """
            ...
            Those entries will have, correspondingly, all the structure's members (IMAGE_RESOURCE_DIRECTORY_ENTRY) and an additional one, "directory", pointing to the IMAGE_RESOURCE_DIRECTORY structure representing upper layers of the tree. This one will also have an 'entries' attribute, pointing to the 3rd, and last, level.
            Another directory with more entries. Those last entries will have a new attribute (both 'leaf' or 'data_entry' can be used to access it). This structure finally points to the resource data.
            All the members of this structure, IMAGE_RESOURCE_DATA_ENTRY, are available as its attributes.
        """
        if dirs is None:
            dirs = [rva]
        ...
        # Get the resource directory structure, that is, the header of the table preceding the actual entries
        ...
        if resource_dir is None:
            ...
            # This directory does not necessarily have to be valid to still have a valid PE file
            self.__warnings.append('Invalid resources directory. Can\'t parse directory data at RVA: 0x%x' % rva)
            return None

        dir_entries = []

        # Advance the RVA to the position immediately following the directory table header and pointing to the first entry in the table
        ...
        # Keep track of the last name's start and end offsets in order to be able to detect overlapping entries that might suggest and invalid or corrupt directory.
        last_name_begin_end = None
        for idx in range(number_of_entries):

            res = self.parse_directory_resources_entry(rva)
            if res is None:
            ...
            if not name_is_string:
                ...
                try:
                    ...
                    # If the last entry's offset points before the current's but its end is past the current's beginning, assume the overlap indicates a corrupt name.
            ...
            if res.DataIsDirectory:
                # One trick malware can do is to recursively reference the next directory.
                # This causes hilarity to ensue when trying to parse everything correctly.
                # If the original RVA given to this function is equal to the next one to parse, we assume that it's a trick.
                # Instead of raising a PEFormatError this would skip some reasonable data so we just break.
                # 9ee4d0a0caf095314fd7041a3e4404dc is the offending sample
                if (base_rva + res.OffsetToDirectory) in dirs:
                    break

                else:
                    entry_directory = self.parse_directory_resources(base_rva + res.OffsetToDirectory, size - (rva - base_rva), base_rva=base_rva, level=level + 1, dirs=dirs + [base_rva + res.OffsetToDirectory])
                ...
                struct = self.parse_directory_resources_item(base_rva + res.OffsetToDirectory)
                if struct:
            ...
            if level == 0 and res.Id == RESOURCE_TYPE['RT_VERSION']:
                ...
                if rt_version_struct is not None:
                    self.parse_directory_resources_version_info(rt_version_struct)
    ...
    def parse_directory_debug(self, rva, size):
        """
            parse dir: debug
        """
        ...
        for idx in range(int(size / dbg_size)):
            ...
            if dbg.Type == 1:
                ...
                if dbg_type_data[:4] == b'RSDS':
                    # pdb7.0
                    __CV_INFO_PDB70_format__ = ['CV_INFO_PDB70', ['I,CvSignature',
                                                                  # Signature is of GUID type
                                                                  'I,Signature_Data1',
                                                                  'H,Signature_Data2',
                                                                  'H,Signature_Data3',
                                                                  'H,Signature_Data4',
                    ...
                    # pdb2.0
                    __CV_INFO_PDB20_format__ = ['CV_INFO_PDB20', ['I,CvHeaderSignature',
                                                                  'I,CvHeaderOffset',
                                                                  'I,Signature',
                                                                  'I,Age']]
                    pdbFileName_size = (dbg_type_size - Structure(__CV_INFO_PDB20_format__).sizeof())

                    # As with the PDB 7.0 case, ensuring a positive size for pdbFileName_size to ensure proper parsing.
                    if pdbFileName_size > 0:
                ...
                dbg_type_size = dbg.SizeOfData
                dbg_type_data = self.__data__[dbg_type_offset:dbg_type_offset + dbg_type_size]
                ___IMAGE_DEBUG_MISC_format__ = ['IMAGE_DEBUG_MISC', ['I,DataType',
                                                                     'I,Length',
                                                                     'B,Unicode',
                                                                     'B,Reserved1',
                                                                     'H,Reserved2']]
                ...
                # Need to check that dbg_type_partial contains a correctly unpacked data structure, as the malware sample with the following hash
                # MD5:    5e7d6707d693108de5a303045c17d95b
                # SHA256: 5dd94a95025f3b6e3dd440d52f7c6d2964fdd1aa119e0ee92e38c7bf83829e5c
                # contains a value of None for dbg_type_partial after unpacking, presumably due to a malformed DEBUG entry.
                if dbg_type_partial:
                    ...
                    if dbg_type_partial.Unicode in (0, 1):
                        ...
                        # As with the PDB case, ensuring a positive size for data_size here to ensure proper parsing.
                        if data_size > 0:
    ...
    def parse_directory_relocations_item(self, data_rva, rva, size):
        """
            parse relocations
        """
        try:
        ...
        for idx in range(int(len(data) / 2)):
            ...
            if (reloc_offset, reloc_type) in offsets_and_type:
                self.__warnings.append('Overlapping offsets in relocation data data at RVA: 0x%x' % (reloc_offset + rva))
                break
    ...
    def parse_directory_relocations(self, rva, size):
        """
            parse dir: relocations
        """
        ...
        while rva < end:
            ...
            reloc_entries = self.parse_directory_relocations_item(rva + rlc_size, rlc.VirtualAddress, rlc.SizeOfBlock - rlc_size)
    ...
    def parse_directory_tls(self, rva, size):
        """
            ...
            By default let's pretend the format is a 32-bit PE.
            It may help produce some output for files where the Magic in the Optional Header is incorrect.
        """
    ...
    def parse_directory_bound_imports(self, rva, size):
        ...
        while True:
            ...
            for idx in range(min(bnd_descr.NumberOfModuleForwarderRefs, int(safety_boundary / 8))):
                # Both structures IMAGE_BOUND_IMPORT_DESCRIPTOR and IMAGE_BOUND_FORWARDER_REF have the same size.
                ...
                # Anything longer than a safety length of 128 will be taken to indicate a corrupt entry and abort the processing of these entries.
    ...
    def get_import_table(self):
        """
            get parsed import table

            @return: LIST: list of obj ImportDescData
                     None
        """
        return hasattr(self, "DIRECTORY_ENTRY_IMPORT") and self.DIRECTORY_ENTRY_IMPORT or None

    def get_delay_import_table(self):
        """
            get parsed delay import table

            @return: LIST: list of obj ImportDescData
                     None
        """
        return hasattr(self, "DIRECTORY_ENTRY_DELAY_IMPORT") and self.DIRECTORY_ENTRY_DELAY_IMPORT or None

    def get_bound_import_table(self):
        """
            get parsed bound import table

            @return: LIST: list of obj ImportDescData
                     None
        """
        return hasattr(self, "DIRECTORY_ENTRY_BOUND_IMPORT") and self.DIRECTORY_ENTRY_BOUND_IMPORT or None

    def get_export_table(self):
        """
            get parsed export table

            @return: obj of ExportDirData
                     None
        """
    ...
    def get_memory_mapped_image(self, max_virtual_address=0x10000000, ImageBase=None):
        """
            ...
            The data includes the PE header and the sections loaded at offsets corresponding to their relative virtual addresses. (the VirtualAddress section header member).
            Any offset in this data corresponds to the absolute memory address ImageBase+offset.

            The optional argument 'max_virtual_address' provides with means of limiting which sections are processed.
            Any section with their VirtualAddress beyond this value will be skipped.
            Normally, sections with values beyond this range are just there to confuse tools. It's a common trick to see in packed executables.

            If the 'ImageBase' optional argument is supplied, the file's relocations will be applied to the image by calling the 'relocate_image()' method.
            Beware that the relocation information is applied permanently.
        """
        # Rebase if requested
    ...
    def get_resources_strings(self):
        """
            get strings found withing the resources (if any).
    ...
    def get_data(self, rva=0, length=None):
        ...
        if not s:
            ...
            # Before we give up we check whether the file might contain the data anyway.
            # There are cases of PE files without sections that rely on windows loading the first 8291 bytes into memory and assume the data will be there
    ...
    def get_rva_from_offset(self, offset):
        ...
        if not s:
            if self.sections:
                lowest_rva = min([self.adjust_SectionAlignment(section.VirtualAddress, self.OPTIONAL_HEADER.SectionAlignment, self.OPTIONAL_HEADER.FileAlignment) for section in self.sections])
                if offset < lowest_rva:
                    # We will assume that the offset lies within the headers, or at least points before where the earliest section starts and we will simply return the offset as the RVA
                    # The case illustrating this behavior can be found at:
                    # http://corkami.blogspot.com/2010/01/hey-hey-hey-whats-in-your-head.html
                    # where the import table is not contained by any section hence the RVA needs to be resolved to a raw offset
                    return offset
                return None
            else:
                return offset
    ...
    def get_offset_from_rva(self, rva):
        ...
        s = self.get_section_by_rva(rva)
        if not s:
            # If not found within a section assume it might point to overlay data or otherwise data present but not contained in any section.
    ...
    def get_string_u_at_rva(self, rva, max_length=2**16, encoding=None):
        ...
        # except PEFormatError as e:
            ...
            return None
        # max_length is the maximum count of 16bit characters needs to be doubled to get size in bytes
        max_length <<= 1
    ...
    def get_dword_from_data(self, data, offset):
        """
            ...
            'offset' is assumed to index into a dword array. So setting it to N will return a dword out of the data starting at offset N*4.
    ...
    def get_word_from_data(self, data, offset):
        """
            ...
            'offset' is assumed to index into a word array. So setting it to N will return a dword out of the data starting at offset N*2.
    ...
    def get_qword_from_data(self, data, offset):
        """
            ...
            'offset' is assumed to index into a word array. So setting it to N will return a dword out of the data starting at offset N*8.
    ...
    def dump_info(self, dump=None, encoding='ascii'):
                ...
                for entry in self.FileInfo:
                    ...
                    if hasattr(entry, 'StringTable'):
                        for st_entry in entry.StringTable:
                            ...
                            dump.add_line(u'  LangID: {0}'.format(st_entry.LangID.decode(encoding, 'backslashreplace_')))
                            dump.add_newline()
                            for str_entry in sorted(list(st_entry.entries.items())):
                                # try:
                                dump.add_line(u'    {0}: {1}'.format(str_entry[0].decode(encoding, 'backslashreplace_'), str_entry[1].decode(encoding, 'backslashreplace_')))
                        ...
                        for var_entry in entry.Var:
                            if hasattr(var_entry, 'entry'):
                                ...
                                dump.add_line(u'    {0}: {1}'.format(list(var_entry.entry.keys())[0].decode('utf-8', 'backslashreplace'), list(var_entry.entry.values())[0]))
                        dump.add_newline()
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_EXPORT'):
            ...
            for export in self.DIRECTORY_ENTRY_EXPORT.symbols:
                if export.address is not None:
                    ...
                    if export.name:
                        name = export.name
                    dump.add(u'%-10d 0x%08Xh    %s' % (export.ordinal, export.address, name.decode(encoding)))
                    if export.forwarder:
                        dump.add_line(u' forwarder: {0}'.format(export.forwarder.decode(encoding)))
                    else:
                        dump.add_newline()
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_IMPORT'):
            ...
            for module in self.DIRECTORY_ENTRY_IMPORT:
                ...
                if not module.imports:
                    dump.add('  Name -> {0}'.format(self.get_string_at_rva(module.struct.Name).decode(encoding)))
                    dump.add_newline()
                dump.add_newline()
                for symbol in module.imports:
                    if symbol.import_by_ordinal is True:
                        ...
                        dump.add('{0}.{1} Hint[{2:d}]'.format(module.dll.decode(encoding), symbol.name.decode(encoding), symbol.hint))
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_BOUND_IMPORT'):
            ...
            for bound_imp_desc in self.DIRECTORY_ENTRY_BOUND_IMPORT:
                ...
                dump.add_line('DLL: {0}'.format(bound_imp_desc.name.decode(encoding)))
                dump.add_newline()

                for bound_imp_ref in bound_imp_desc.entries:
                    dump.add_lines(bound_imp_ref.struct.dump(), 4)
                    dump.add_line('DLL: {0}'.format(bound_imp_ref.name.decode(encoding)), 4)
                    dump.add_newline()
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_DELAY_IMPORT'):
            ...
            for module in self.DIRECTORY_ENTRY_DELAY_IMPORT:
                ...
                for symbol in module.imports:
                    if symbol.import_by_ordinal is True:
                        dump.add('{0} Ordinal[{1:d}] (Imported by Ordinal)'.format(module.dll.decode(encoding), symbol.ordinal))
                    else:
                        dump.add('{0}.{1} Hint[{2}]'.format(module.dll.decode(encoding), symbol.name.decode(encoding), symbol.hint))
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_RESOURCE'):
            ...
            for resource_type in self.DIRECTORY_ENTRY_RESOURCE.entries:
                ...
                if resource_type.name is not None:
                    ...
                    dump.add_line(u'Name: [{0}]'.format(resource_type.name.decode(encoding, 'backslashreplace')), 2)
                else:
                    dump.add_line(u'Id: [0x{0:X}] ({1})'.format(resource_type.struct.Id, RESOURCE_TYPE.get(resource_type.struct.Id, '-')), 2)
                ...
                if hasattr(resource_type, 'directory'):
                    ...
                    for resource_id in resource_type.directory.entries:
                        ...
                        if resource_id.name is not None:
                            dump.add_line(u'Name: [{0}]'.format(resource_id.name.decode('utf-8', 'backslashreplace')), 6)
                        else:
                        ...
                        if hasattr(resource_id, 'directory'):
                            ...
                            for resource_lang in resource_id.directory.entries:
                                if hasattr(resource_lang, 'data'):
                                    dump.add_line(u'\\--- LANG [%d,%d][%s,%s]' % (resource_lang.data.lang, resource_lang.data.sublang, LANG.get(resource_lang.data.lang, '*unknown*'), get_sublang_name_for_lang(resource_lang.data.lang, resource_lang.data.sublang)), 8)
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_RESOURCE'):
            ...
            for resource_type in self.DIRECTORY_ENTRY_RESOURCE.entries:
                ...
                if resource_type.name is not None:
                    ...
                    resource_type_dict['Id'] = (resource_type.struct.Id, RESOURCE_TYPE.get(resource_type.struct.Id, '-'))
    ...
    def is_driver(self):
        ...
        # If the import directory was not parsed (v_fast_load = True); do it now.
        if not hasattr(self, 'DIRECTORY_ENTRY_IMPORT'):
            self.parse_data_directories(directories=[DIRECTORY_ENTRY['IMAGE_DIRECTORY_ENTRY_IMPORT']])

        # If there's still no import directory (the PE doesn't have one or it's malformed), give up.
        if not hasattr(self, 'DIRECTORY_ENTRY_IMPORT'):
            return False
    ...
    def generate_checksum(self):
        ...
        # Get the offset to the CheckSum field in the OptionalHeader (The offset is the same in PE32 and PE32+) 64
        checksum_offset = self.OPTIONAL_HEADER.__file_offset__ + 0x40
    ...
    def relocate_image(self, new_ImageBase):
        """
            ...
            This method will apply the relocation information to the image.
            Given the new base, all the relocations will be processed and both the raw data and the section's data will be fixed accordingly.
        ...
        for reloc in self.DIRECTORY_ENTRY_BASERELOC:
            ...
            # We iterate with an index because if the relocation is of type IMAGE_REL_BASED_HIGHADJ we need to also process the next entry at once and skip it for the next iteration
            entry_idx = 0
            while entry_idx < len(reloc.entries):
                ...
                if entry.type == RELOCATION_TYPE['IMAGE_REL_BASED_ABSOLUTE']:
                    ...
                    # Add high 16-bits of relocation_difference to the 16-bit value at RVA=entry.rva
                    ...
                    # Add low 16 bits of relocation_difference to the 16-bit value at RVA=entry.rva
                    ...
                    # Add high 16-bits of relocation_difference to the 32-bit value composed from the (16-bit value at RVA=entry.rva)<<16 plus the 16-bit value at the next relocation entry.
                    # If the next entry is beyond the array's limits, abort... the table is corrupt
                    ...
                    # Apply the difference to the 64-bit value at the offset RVA=entry.rva
    ...
    def get_overlay_data_start_offset(self):
        ...
        if hasattr(self, 'OPTIONAL_HEADER'):
            largest_offset_and_size = update_if_sum_is_larger_and_within_file((self.OPTIONAL_HEADER.get_file_offset(), self.FILE_HEADER.SizeOfOptionalHeader))

        for section in self.sections:
            largest_offset_and_size = update_if_sum_is_larger_and_within_file((section.PointerToRawData, section.SizeOfRawData))

        for directory in self.OPTIONAL_HEADER.DATA_DIRECTORY:
            largest_offset_and_size = update_if_sum_is_larger_and_within_file((directory.VirtualAddress, directory.Size))
    ...
    def get_warnings(self):
        """
            ...
            Non-critical problems found when parsing the PE file are appended to a list of warnings.
            This method returns the full list.
        """
    ...
    def show_warnings(self):
        """
            ...
            Non-critical problems found when parsing the PE file are appended to a list of warnings.
            This method prints the full list to standard output.
        """
    ...
    def full_load(self):
        """
            ...
            This method will load the data directories which might not have been loaded if the "v_fast_load" option was used.
        """
        self.parse_data_directories()
    ...
    def write(self, filename=None):
        """
            ...
            This function will process all headers and components of the PE file and include all changes made (by just assigning to attributes in the PE objects) and write the changes back to a file whose name is provided as an argument.
            The filename is optional, if not provided the data will be returned as a 'str' object.
        """
    ...
    # Hence we reproduce the behavior
    #
