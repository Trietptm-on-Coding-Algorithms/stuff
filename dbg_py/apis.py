...
# is print param log detail when param_log_ctrl.get_log()
...
class param_log_ctrl:

    def __init__(self, esp_offset, log_name, log_type):
        """
            @param: esp_offset : int    : offset to esp
            @param: log_name   : string : log param name
            @param: log_type   : int    : [V_PARAM_LOG_BYTE, ...]
        """
    ...
    def get_log(self, dbg):
        ...
        """
        global v_tmp_is_pt_param_log_detail
        if v_tmp_is_pt_param_log_detail:
            v = dbg.read_stack_int32(self.esp_offset)
            _pt_log(">>> param log detail: %s at %.8X" % (self, v))

        value = ""
        if self.log_type == V_PARAM_LOG_BYTE:
            value = "%d" % dbg.read_stack_int8(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_WORD:
            value = "%.4X" % dbg.read_stack_int16(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_INT:
            value = "%.8X" % dbg.read_stack_int32(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_PBYTE:
            value = "%d" % dbg.read_stack_p_int8(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_PWORD:
            value = "%.4X" % dbg.read_stack_p_int16(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_PINT:
            value = "%.8X" % dbg.read_stack_p_int32(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_ASTR:
            value = dbg.read_stack_ascii_string(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_USTR:
            value = dbg.read_stack_unicode_string(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_PASTR:
            value = dbg.read_stack_p_ascii_string(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_PUSTR:
            value = dbg.read_stack_p_unicode_string(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_PPASTR:
            value = dbg.read_stack_pp_ascii_string(self.esp_offset)

        elif self.log_type == V_PARAM_LOG_PPUSTR:
            value = dbg.read_stack_pp_unicode_string(self.esp_offset)
...
class api_ctrl:
    ...
    def __init__(self, dll_name, api_name, cstk_filter_depth=1, max_invoke_cnt_runtime=None):
        """
            @param: dll_name               : string : dll name api belong to
            @param: api_name               : string : api name
            @param: cskt_filter_depth      : int    : (optional, dft=1)callstack filter depth
            @param: max_invoke_cnt_runtime : int    : (optional, dft=None)when reach limit, remove api bp
        """
...
class api_ctrl_params(api_ctrl):

    def __init__(self, dll_name, api_name, param_log_ctrl_list=None, cstk_filter_depth=1, max_invoke_cnt_runtime=None):
        """
            @param: param_log_ctrl_list : list or None : (optional, dft=None)param log ctrl list
        """
        api_ctrl.__init__(self, dll_name, api_name, cstk_filter_depth=cstk_filter_depth, max_invoke_cnt_runtime=max_invoke_cnt_runtime)
        self.param_log_ctrl_list = param_log_ctrl_list

    def __str__(self):
        return "params(%s)" % api_ctrl.__str__(self)


class api_ctrl_custom(api_ctrl):

    def __init__(self, dll_name, api_name, handler, cstk_filter_depth=1, max_invoke_cnt_runtime=None):
        """
            @param: handler : method : callback to invoke when api hit.
        """
        api_ctrl.__init__(self, dll_name, api_name, cstk_filter_depth=cstk_filter_depth, max_invoke_cnt_runtime=max_invoke_cnt_runtime)
        self.handler = handler

    def __str__(self):
        return "custom(%s)" % api_ctrl.__str__(self)
...
def resolve_call_stacks(stacks):
    """
        ...
        @param: stacks: list : a list of _share_this.call_stack() object

        @return: list : a list of _share_this.call_stack() object
    """
    for stack in stacks:
...
def get_reg_data(dbg, type_, pdata, data_size=0):
    ...
    if type_ == _winreg.REG_BINARY:
        ...
        return "REG_DWORD", "%d" % dbg.read_int32(pdata)
...
def handler_InternetConnectA(dbg):
    """
        ...
        InternetConnectW-->InternetConnectA-->FtpConnect/HttpConnect
          ...
          param_log_ctrl(0x14, "user_pwd", V_PARAM_LOG_PASTR)]
    """
...
def handler_DllFunctionCall(dbg):
    p = dbg.read_stack_int32(4)
    p_dll_name = dbg.read_int32(p)
    p_func_name = dbg.read_int32(p + 4)
    dll_name = dbg.read_ascii_string(p_dll_name)
    func_name = dbg.read_ascii_string(p_func_name)

    param_dict = {"dll_name": dll_name, "func_name": func_name}
    _xrk_api_invoke_detail(dbg, "DllFunctionCall", param_dict)
    return param_dict
...
# func list
func_reg_advapi32 = [
    api_ctrl_params("advapi32.dll", "RegCreateKeyExA", [param_log_ctrl(8, "key", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "RegCreateKeyExW", [param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "RegConnectRegistryW", [param_log_ctrl(4, "key", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_custom("advapi32.dll", "RegSetValueExA", handler=handler_RegSetValueExA, cstk_filter_depth=2),
    api_ctrl_custom("advapi32.dll", "RegSetValueExW", handler=handler_RegSetValueExW, cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "RegDeleteKeyA", [param_log_ctrl(8, "key", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "RegDeleteKeyW", [param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "RegDeleteValueA", [param_log_ctrl(8, "key", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "RegDeleteValueW", [param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "RegSaveKeyExA", [param_log_ctrl(8, "key", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "RegSaveKeyExW", [param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "RegSaveKeyA", [param_log_ctrl(8, "key", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "RegSaveKeyW", [param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "RegReplaceKeyA", [param_log_ctrl(8, "key", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "file_new", V_PARAM_LOG_PASTR), param_log_ctrl(0x10, "file_old", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "RegReplaceKeyW", [param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "file_new", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "file_old", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "RegRestoreKeyA", [param_log_ctrl(8, "key", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "RegRestoreKeyW", [param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "RegLoadKeyA", [param_log_ctrl(8, "key", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "file", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "RegLoadKeyW", [param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "file", V_PARAM_LOG_PUSTR)]),
]

func_reg_advapi32_fragile = [
    api_ctrl_params("advapi32.dll", "RegOpenKeyExA", [param_log_ctrl(8, "key", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "RegOpenKeyExW", [param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "RegQueryInfoKeyA", max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegQueryInfoKeyW", max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegQueryMultipleValuesA", max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegQueryMultipleValuesW", max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegQueryValueExA", [param_log_ctrl(8, "vname", V_PARAM_LOG_PASTR)], cstk_filter_depth=2, max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegQueryValueExW", [param_log_ctrl(8, "vname", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2, max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegEnumKeyExA", cstk_filter_depth=2, max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegEnumKeyW", max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegEnumKeyExW", max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegEnumValueA", max_invoke_cnt_runtime=10),
    api_ctrl_params("advapi32.dll", "RegEnumValueW", max_invoke_cnt_runtime=10),
]

func_dns_dnsapi = [
    api_ctrl_params("dnsapi.dll", "DnsQuery_W", [param_log_ctrl(4, "name", V_PARAM_LOG_PUSTR)], cstk_filter_depth=4),
    api_ctrl_params("dnsapi.dll", "DnsQuery_UTF8", [param_log_ctrl(4, "name", V_PARAM_LOG_PASTR)]),
]

func_sock_ws2_32 = [
    api_ctrl_params("ws2_32.dll", "WSAStartup"),
    api_ctrl_params("ws2_32.dll", "WSACleanup"),
    api_ctrl_custom("ws2_32.dll", "WSASocketW", handler=handler_WSASocketW, cstk_filter_depth=2),
    api_ctrl_params("ws2_32.dll", "closesocket"),
    api_ctrl_params("ws2_32.dll", "getnameinfo"),
    api_ctrl_params("ws2_32.dll", "GetNameInfoW"),
    api_ctrl_custom("ws2_32.dll", "getsockname", handler=handler_getsockname),
    api_ctrl_params("ws2_32.dll", "getpeername"),
    api_ctrl_custom("ws2_32.dll", "gethostname", handler=handler_gethostname),
    api_ctrl_params("ws2_32.dll", "gethostbyaddr"),
    api_ctrl_custom("ws2_32.dll", "gethostbyname", handler=handler_gethostbyname),
    api_ctrl_params("ws2_32.dll", "getaddrinfo", [param_log_ctrl(4, "node", V_PARAM_LOG_PASTR), param_log_ctrl(8, "svc", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("ws2_32.dll", "GetAddrInfoW", [param_log_ctrl(4, "node", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "svc", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("ws2_32.dll", "freeaddrinfo"),
    api_ctrl_custom("ws2_32.dll", "bind", handler=handler_bind),
    api_ctrl_params("ws2_32.dll", "listen"),
    api_ctrl_custom("ws2_32.dll", "connect", handler=handler_connect),
    api_ctrl_custom("ws2_32.dll", "send", handler=handler_send),
    api_ctrl_custom("ws2_32.dll", "sendto", handler=handler_sendto),
    api_ctrl_custom("ws2_32.dll", "recv", handler=handler_recv),
    api_ctrl_custom("ws2_32.dll", "recvfrom", handler=handler_recvfrom),
    api_ctrl_custom("ws2_32.dll", "select", handler=handler_select),
    api_ctrl_custom("ws2_32.dll", "setsockopt", handler=handler_setsockopt),
]

func_sock_ws2_32_WSA = [
    api_ctrl_params("ws2_32.dll", "WSAAccept", cstk_filter_depth=2),
    api_ctrl_custom("ws2_32.dll", "WSASend", handler=handler_WSASend),
    api_ctrl_custom("ws2_32.dll", "WSASendTo", handler=handler_WSASendTo),
    api_ctrl_custom("ws2_32.dll", "WSAConnect", handler=handler_WSAConnect),
    api_ctrl_params("ws2_32.dll", "WSASendDisconnect"),
    api_ctrl_custom("ws2_32.dll", "WSARecv", handler=handler_WSARecv),
    api_ctrl_custom("ws2_32.dll", "WSARecvFrom", handler=handler_WSARecvFrom),
    api_ctrl_params("ws2_32.dll", "WSARecvDisconnect"),
]

func_internet_wininet = [
    api_ctrl_params("wininet.dll", "InternetOpenA", [param_log_ctrl(4, "agent", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "proxy_name", V_PARAM_LOG_PASTR), param_log_ctrl(0x10, "proxy_pwd", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_custom("wininet.dll", "InternetConnectA", handler=handler_InternetConnectA, cstk_filter_depth=2),
    api_ctrl_params("wininet.dll", "InternetCrackUrlA", [param_log_ctrl(4, "url", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_params("wininet.dll", "InternetOpenUrlA", [param_log_ctrl(8, "url", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "headers", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_params("wininet.dll", "InternetReadFile"),
    api_ctrl_params("wininet.dll", "InternetReadFileExA", cstk_filter_depth=2),
    api_ctrl_params("wininet.dll", "InternetWriteFile"),
    api_ctrl_params("wininet.dll", "HttpOpenRequestA", [param_log_ctrl(8, "verb", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "obj", V_PARAM_LOG_PASTR), param_log_ctrl(0x10, "ver", V_PARAM_LOG_PASTR), param_log_ctrl(0x14, "refer", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("wininet.dll", "HttpOpenRequestW", [param_log_ctrl(8, "verb", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "obj", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "ver", V_PARAM_LOG_PUSTR), param_log_ctrl(0x14, "refer", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("wininet.dll", "HttpSendRequestA", [param_log_ctrl(8, "headers", V_PARAM_LOG_PASTR), param_log_ctrl(0x10, "opt", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("wininet.dll", "HttpSendRequestW", [param_log_ctrl(8, "headers", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "opt", V_PARAM_LOG_PUSTR)]),
    api_ctrl_custom("wininet.dll", "HttpSendRequestExA", handler=handler_HttpSendRequestExA),
    api_ctrl_custom("wininet.dll", "HttpSendRequestExW", handler=handler_HttpSendRequestExW),
    api_ctrl_params("wininet.dll", "HttpAddRequestHeadersA", cstk_filter_depth=2),
]

func_internet_wininet_1 = [
    api_ctrl_params("wininet.dll", "InternetFindNextFileA", cstk_filter_depth=2),
    api_ctrl_params("wininet.dll", "InternetGetCookieExW", [param_log_ctrl(4, "url", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "coockie", V_PARAM_LOG_PUSTR)], cstk_filter_depth=3),
    api_ctrl_params("wininet.dll", "InternetSetCookieA", [param_log_ctrl(4, "url", V_PARAM_LOG_PASTR), param_log_ctrl(8, "coockie_name", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "coockie_data", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_params("wininet.dll", "InternetSetCookieExA", [param_log_ctrl(4, "url", V_PARAM_LOG_PASTR), param_log_ctrl(8, "coockie_name", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "coockie_data", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("wininet.dll", "InternetSetCookieExW", [param_log_ctrl(4, "url", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "coockie_name", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "coockie_data", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("wininet.dll", "InternetAttemptConnect"),
    api_ctrl_params("wininet.dll", "InternetCanonicalizeUrlA", [param_log_ctrl(4, "url", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("wininet.dll", "InternetCanonicalizeUrlW", [param_log_ctrl(4, "url", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("wininet.dll", "DeleteUrlCacheEntryA", [param_log_ctrl(4, "url", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
]

func_internet_winhttp = [
    api_ctrl_params("winhttp.dll", "WinHttpOpen", [param_log_ctrl(4, "agent", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "proxy_name", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "proxy_pwd", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("winhttp.dll", "WinHttpCloseHandle"),
    api_ctrl_params("winhttp.dll", "WinHttpConnect", [param_log_ctrl(8, "svr_name", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "svr_port", V_PARAM_LOG_INT)]),
    api_ctrl_params("winhttp.dll", "WinHttpOpenRequest", [param_log_ctrl(8, "verb", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "obj", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "ver", V_PARAM_LOG_PUSTR), param_log_ctrl(0x14, "refer", V_PARAM_LOG_PUSTR), param_log_ctrl(0x18, "acept_type", V_PARAM_LOG_PPUSTR)]),
    api_ctrl_params("winhttp.dll", "WinHttpSendRequest", [param_log_ctrl(8, "headers", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("winhttp.dll", "WinHttpReceiveResponse"),
    api_ctrl_params("winhttp.dll", "WinHttpQueryHeaders", [param_log_ctrl(0xC, "name", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("winhttp.dll", "WinHttpQueryDataAvailable"),
    api_ctrl_params("winhttp.dll", "WinHttpReadData"),
    api_ctrl_params("winhttp.dll", "WinHttpAddRequestHeaders", [param_log_ctrl(8, "headers", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("winhttp.dll", "WinHttpCrackUrl", [param_log_ctrl(4, "url", V_PARAM_LOG_PUSTR)]),
    api_ctrl_custom("winhttp.dll", "WinHttpCreateUrl", handler=handler_WinHttpCreateUrl),
    api_ctrl_custom("winhttp.dll", "WinHttpWriteData", handler=handler_WinHttpWriteData),
]

func_internet_urlmon = [
    api_ctrl_params("urlmon.dll", "URLDownloadW", [param_log_ctrl(8, "url", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("urlmon.dll", "URLDownloadToFileW", [param_log_ctrl(8, "url", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("urlmon.dll", "URLDownloadToCacheFileW", [param_log_ctrl(8, "url", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
]

func_internet_rasapi32 = [
    api_ctrl_params("rasapi32.dll", "RasGetConnectStatusW", cstk_filter_depth=2),
]

func_proc_kernel32 = [
    api_ctrl_custom("kernel32.dll", "IsWow64Process", handler=handler_IsWow64Process),
    api_ctrl_custom("kernel32.dll", "CreateProcessInternalW", handler=handler_CreateProcessInternalW, cstk_filter_depth=3),
    api_ctrl_custom("kernel32.dll", "CreateRemoteThread", handler=handler_CreateRemoteThread, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "OpenProcess", handler=handler_OpenProcess),
    api_ctrl_custom("kernel32.dll", "GetExitCodeProcess", handler=handler_GetExitCodeProcess),
    api_ctrl_params("kernel32.dll", "OpenThread", [param_log_ctrl(4, "tid", V_PARAM_LOG_INT)]),
    api_ctrl_params("kernel32.dll", "TerminateThread", [param_log_ctrl(8, "code", V_PARAM_LOG_INT)]),
    api_ctrl_params("kernel32.dll", "SuspendThread"),
    api_ctrl_params("kernel32.dll", "ResumeThread"),
    api_ctrl_params("kernel32.dll", "ExitThread", [param_log_ctrl(4, "code", V_PARAM_LOG_INT)]),
]

func_proc_kernel32_1 = [
    api_ctrl_custom("kernel32.dll", "SetThreadContext", handler=handler_SetThreadContext),
    api_ctrl_custom("kernel32.dll", "GetThreadContext", handler=handler_GetThreadContext),
    api_ctrl_custom("kernel32.dll", "ReadProcessMemory", handler=handler_ReadProcessMemory, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "WriteProcessMemory", handler=handler_WriteProcessMemory),
    api_ctrl_custom("kernel32.dll", "CreateToolhelp32Snapshot", handler=handler_CreateToolhelp32Snapshot),
]

func_proc_kernel32_2 = [
    api_ctrl_params("kernel32.dll", "Process32FirstW", cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "Process32NextW", cstk_filter_depth=2, max_invoke_cnt_runtime=10),
    api_ctrl_params("kernel32.dll", "Module32FirstW", cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "Module32NextW", cstk_filter_depth=2, max_invoke_cnt_runtime=10),
    api_ctrl_params("kernel32.dll", "Thread32First"),
    api_ctrl_params("kernel32.dll", "Thread32Next", max_invoke_cnt_runtime=10),
]

func_file_kernel32 = [
    api_ctrl_custom("kernel32.dll", "CreateFileMappingW", handler=handler_CreateFileMappingW, cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "MapViewOfFileEx"),
    api_ctrl_custom("kernel32.dll", "UnmapViewOfFile", handler=handler_UnmapViewOfFile),
    api_ctrl_custom("kernel32.dll", "CreateFileW", handler=handler_CreateFileW, cstk_filter_depth=3),
    api_ctrl_params("kernel32.dll", "ReadFile", cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "ReadFileEx"),
    api_ctrl_custom("kernel32.dll", "WriteFile", handler=handler_WriteFile, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "WriteFileEx", handler=handler_WriteFileEx),
    api_ctrl_params("kernel32.dll", "CopyFileExW", [param_log_ctrl(4, "file_old", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "file_new", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "MoveFileWithProgressW", handler=handler_MoveFileWithProgressW, cstk_filter_depth=3),
    api_ctrl_params("kernel32.dll", "CreateDirectoryW", [param_log_ctrl(4, "dir", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "CreateDirectoryExW", [param_log_ctrl(4, "dir_template", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "dir_new", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "RemoveDirectoryW", handler=handler_RemoveDirectoryW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "ReplaceFileW", handler=handler_ReplaceFileW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "DeleteFileW", handler=handler_DeleteFileW, cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "DeviceIoControl", [param_log_ctrl(8, "code", V_PARAM_LOG_INT)]),
    api_ctrl_params("kernel32.dll", "FindFirstFileExW", [param_log_ctrl(4, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "SetFileAttributesW", handler=handler_SetFileAttributesW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "SetFileTime", handler=handler_SetFileTime),
    api_ctrl_params("kernel32.dll", "GetFileTime"),
    api_ctrl_params("kernel32.dll", "GetFileSizeEx", cstk_filter_depth=2),

    api_ctrl_custom("kernel32.dll", "GetTempPathW", handler=handler_GetTempPathW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "GetTempFileNameW", handler=handler_GetTempFileNameW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "GetSystemDirectoryA", handler=handler_GetSystemDirectoryA),
    api_ctrl_custom("kernel32.dll", "GetSystemDirectoryW", handler=handler_GetSystemDirectoryW),
]

func_file_kernel32_1 = [
    api_ctrl_params("kernel32.dll", "GetDiskFreeSpaceW", [param_log_ctrl(4, "path", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "GetDiskFreeSpaceExW", [param_log_ctrl(4, "path", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "GetDriveTypeW", [param_log_ctrl(4, "path", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "GetVolumeInformationW", [param_log_ctrl(4, "path", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "GetVolumeNameForVolumeMountPointW", [param_log_ctrl(4, "path", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "FindFirstVolumeW", cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "FindNextVolumeW", cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "GetFullPathNameW", handler=handler_GetFullPathNameW, cstk_filter_depth=3),
    api_ctrl_params("kernel32.dll", "GetVolumePathNamesForVolumeNameW", [param_log_ctrl(4, "path", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "GetLogicalDriveStringsA"),
    api_ctrl_params("kernel32.dll", "GetLogicalDriveStringsW"),
    api_ctrl_params("kernel32.dll", "GetLogicalDrives"),
    api_ctrl_params("kernel32.dll", "FindNextFileW", cstk_filter_depth=2, max_invoke_cnt_runtime=10),
    api_ctrl_params("kernel32.dll", "SetDllDirectoryA", [param_log_ctrl(4, "dir", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "SetDllDirectoryW", [param_log_ctrl(4, "dir", V_PARAM_LOG_PUSTR)]),

    # cause sample D0B78005C40EFC219CC1299A57B85C03 HANG
    api_ctrl_params("kernel32.dll", "OpenFileMappingW", [param_log_ctrl(0xC, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
]

func_file_shlwapi = [
    api_ctrl_params("shlwapi.dll", "PathFileExistsA", [param_log_ctrl(4, "path", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("shlwapi.dll", "PathFileExistsW", [param_log_ctrl(4, "path", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("shlwapi.dll", "PathRemoveFileSpecA", [param_log_ctrl(4, "path", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("shlwapi.dll", "PathRemoveFileSpecW", [param_log_ctrl(4, "path", V_PARAM_LOG_PUSTR)]),
]

func_svc_advapi32 = [
    api_ctrl_params("advapi32.dll", "OpenSCManagerA", [param_log_ctrl(4, "machine", V_PARAM_LOG_PASTR), param_log_ctrl(8, "database", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "OpenSCManagerW", [param_log_ctrl(4, "machine", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "database", V_PARAM_LOG_PUSTR)]),
    api_ctrl_custom("advapi32.dll", "CreateServiceA", handler=handler_CreateServiceA),
    api_ctrl_custom("advapi32.dll", "CreateServiceW", handler=handler_CreateServiceW),
    api_ctrl_custom("advapi32.dll", "ControlService", handler=handler_ControlService),
    api_ctrl_params("advapi32.dll", "DeleteService"),
    api_ctrl_params("advapi32.dll", "GetServiceDisplayNameA", [param_log_ctrl(8, "svc", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "GetServiceDisplayNameW", [param_log_ctrl(8, "svc", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "GetServiceKeyNameA", [param_log_ctrl(8, "display_name", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "GetServiceKeyNameW", [param_log_ctrl(8, "display_name", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "OpenServiceA", [param_log_ctrl(8, "svc", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "OpenServiceW", [param_log_ctrl(8, "svc", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "RegisterServiceCtrlHandlerW", [param_log_ctrl(4, "svc", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "cbk", V_PARAM_LOG_INT)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "RegisterServiceCtrlHandlerExW", [param_log_ctrl(4, "svc", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "cbk", V_PARAM_LOG_INT)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "StartServiceA"),
    api_ctrl_params("advapi32.dll", "StartServiceW"),
    api_ctrl_custom("advapi32.dll", "StartServiceCtrlDispatcherA", handler=handler_StartServiceCtrlDispatcherA),
    api_ctrl_custom("advapi32.dll", "StartServiceCtrlDispatcherW", handler=handler_StartServiceCtrlDispatcherW),
]

func_crypto_advapi32 = [
    api_ctrl_params("advapi32.dll", "CryptAcquireContextA", [param_log_ctrl(4, "container", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "provider", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "CryptReleaseContext"),
]

func_crypto_advapi32_1 = [
    api_ctrl_params("advapi32.dll", "CryptSetProvParam"),
    api_ctrl_params("advapi32.dll", "CryptGetProvParam"),
    api_ctrl_params("advapi32.dll", "CryptCreateHash"),
    api_ctrl_params("advapi32.dll", "CryptHashData"),
    api_ctrl_params("advapi32.dll", "CryptGetHashParam"),
    api_ctrl_params("advapi32.dll", "CryptSetHashParam"),
    api_ctrl_params("advapi32.dll", "CryptHashSessionKey"),
    api_ctrl_params("advapi32.dll", "CryptDestroyHash"),
    api_ctrl_params("advapi32.dll", "CryptGenRandom"),
    api_ctrl_params("advapi32.dll", "CryptDeriveKey"),
    api_ctrl_params("advapi32.dll", "CryptGenKey"),
    api_ctrl_params("advapi32.dll", "CryptDestroyKey"),
    api_ctrl_params("advapi32.dll", "CryptImportKey"),
    api_ctrl_params("advapi32.dll", "CryptExportKey"),
    api_ctrl_params("advapi32.dll", "CryptGetKeyParam"),
    api_ctrl_params("advapi32.dll", "CryptSetKeyParam"),
    api_ctrl_params("advapi32.dll", "CryptGetUserKey"),
    api_ctrl_params("advapi32.dll", "CryptSignHashA"),
    api_ctrl_params("advapi32.dll", "CryptSignHashW"),
    api_ctrl_params("advapi32.dll", "CryptVerifySignatureA"),
    api_ctrl_params("advapi32.dll", "CryptVerifySignatureW"),
    api_ctrl_params("advapi32.dll", "CryptEncrypt"),
    api_ctrl_params("advapi32.dll", "CryptDecrypt"),
    api_ctrl_params("advapi32.dll", "CryptDuplicateHash"),
    api_ctrl_params("advapi32.dll", "CryptDuplicateKey"),
]

func_str_kernel32 = [
    api_ctrl_params("kernel32.dll", "lstrcatA", [param_log_ctrl(4, "str1", V_PARAM_LOG_PASTR), param_log_ctrl(8, "str2", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "lstrcatW", [param_log_ctrl(4, "str1", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "str2", V_PARAM_LOG_PUSTR)]),
    api_ctrl_custom("kernel32.dll", "lstrcmpA", handler=handler_lstrcmpA),
    api_ctrl_custom("kernel32.dll", "lstrcmpW", handler=handler_lstrcmpW),
    api_ctrl_custom("kernel32.dll", "lstrcmpiA", handler=handler_lstrcmpiA),
    api_ctrl_custom("kernel32.dll", "lstrcmpiW", handler=handler_lstrcmpiW),
    api_ctrl_params("kernel32.dll", "lstrcpyA", [param_log_ctrl(8, "str2", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "lstrcpyW", [param_log_ctrl(8, "str2", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("kernel32.dll", "lstrcpynA", [param_log_ctrl(8, "str2", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "len", V_PARAM_LOG_INT)]),
    api_ctrl_params("kernel32.dll", "lstrcpynW", [param_log_ctrl(8, "str2", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "len", V_PARAM_LOG_INT)]),
    api_ctrl_params("kernel32.dll", "lstrlenA", [param_log_ctrl(4, "str", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "lstrlenW", [param_log_ctrl(4, "str", V_PARAM_LOG_PUSTR)]),
]

func_str_shlwapi = [
    # todo: there are 2 deeper functions: wvnsprintfA/W
    api_ctrl_custom("shlwapi.dll", "wnsprintfA", handler=handler_wnsprintfA, cstk_filter_depth=2),
    api_ctrl_custom("shlwapi.dll", "wnsprintfW", handler=handler_wnsprintfW, cstk_filter_depth=2),
]

func_str_ntdll = [
    # bp at these apis will result in unknow results...
    api_ctrl_params("ntdll.dll", "RtlInitString", [param_log_ctrl(8, "str", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("ntdll.dll", "RtlInitAnsiString", [param_log_ctrl(8, "str", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("ntdll.dll", "RtlInitUnicodeString", [param_log_ctrl(8, "str", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("ntdll.dll", "RtlInitUnicodeStringEx", [param_log_ctrl(8, "str", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("ntdll.dll", "RtlIsDosDeviceName_U", [param_log_ctrl(4, "str", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("ntdll.dll", "RtlDosPathNameToNtPathName_U", [param_log_ctrl(4, "str", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("ntdll.dll", "RtlDetermineDosPathNameType_U"),
]

func_win_user32 = [
    api_ctrl_params("user32.dll", "FindWindowA", [param_log_ctrl(4, "class_name", V_PARAM_LOG_PASTR), param_log_ctrl(8, "win_name", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("user32.dll", "FindWindowW", [param_log_ctrl(4, "class_name", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "win_name", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("user32.dll", "FindWindowExA", [param_log_ctrl(0xC, "class_name", V_PARAM_LOG_PASTR), param_log_ctrl(0x10, "win_name", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("user32.dll", "FindWindowExW", [param_log_ctrl(0xC, "class_name", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "win_name", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("user32.dll", "GetDesktopWindow"),

    api_ctrl_params("user32.dll", "DialogBoxParamA", [param_log_ctrl(8, "template", V_PARAM_LOG_PASTR), param_log_ctrl(0x10, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "DialogBoxParamW", [param_log_ctrl(8, "template", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "MessageBoxTimeoutW", [param_log_ctrl(8, "txt", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "caption", V_PARAM_LOG_PUSTR), param_log_ctrl(0x18, "mescs", V_PARAM_LOG_INT)], cstk_filter_depth=4),
    api_ctrl_custom("user32.dll", "MessageBoxIndirectA", handler=handler_MessageBoxIndirectA),
    api_ctrl_custom("user32.dll", "MessageBoxIndirectW", handler=handler_MessageBoxIndirectW),
    api_ctrl_custom("user32.dll", "RegisterClassA", handler=handler_RegisterClassA),
    api_ctrl_custom("user32.dll", "RegisterClassW", handler=handler_RegisterClassW),
    api_ctrl_custom("user32.dll", "RegisterClassExA", handler=handler_RegisterClassExA),
    api_ctrl_custom("user32.dll", "RegisterClassExW", handler=handler_RegisterClassExW),
    api_ctrl_params("user32.dll", "PostQuitMessage", [param_log_ctrl(4, "code", V_PARAM_LOG_INT)]),
]

func_win_user32_2 = [
    api_ctrl_params("user32.dll", "CreateWindowExA", [param_log_ctrl(8, "class", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "win", V_PARAM_LOG_PASTR), param_log_ctrl(0x14, "x", V_PARAM_LOG_INT), param_log_ctrl(0x18, "y", V_PARAM_LOG_INT), param_log_ctrl(0x1C, "width", V_PARAM_LOG_INT), param_log_ctrl(0x20, "height", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "CreateWindowExW", [param_log_ctrl(8, "class", V_PARAM_LOG_PUSTR), param_log_ctrl(0x14, "x", V_PARAM_LOG_INT), param_log_ctrl(0x18, "y", V_PARAM_LOG_INT), param_log_ctrl(0x1C, "width", V_PARAM_LOG_INT), param_log_ctrl(0x20, "height", V_PARAM_LOG_INT)]),
]

func_win_user32_1 = [
    api_ctrl_params("user32.dll", "EnumWindows", [param_log_ctrl(4, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "EnumChildWindows", [param_log_ctrl(8, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "EnumDesktopWindows", [param_log_ctrl(8, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "EnumDesktopsA", [param_log_ctrl(8, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "EnumDesktopsW", [param_log_ctrl(8, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "EnumDisplayDevicesA", [param_log_ctrl(4, "file", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("user32.dll", "EnumDisplayDevicesW", [param_log_ctrl(4, "file", V_PARAM_LOG_PUSTR)]),

    api_ctrl_params("user32.dll", "CreateWindowStationA", [param_log_ctrl(4, "win", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("user32.dll", "CreateWindowStationW", [param_log_ctrl(4, "win", V_PARAM_LOG_PUSTR)]),

    api_ctrl_custom("user32.dll", "DispatchMessageA", handler=handler_DispatchMessageA),
    api_ctrl_custom("user32.dll", "DispatchMessageW", handler=handler_DispatchMessageW),
    api_ctrl_params("user32.dll", "PeekMessageA"),
    api_ctrl_params("user32.dll", "PeekMessageW"),
    api_ctrl_custom("user32.dll", "PostMessageA", handler=handler_PostMessageA),
    api_ctrl_custom("user32.dll", "PostMessageW", handler=handler_PostMessageW),
    api_ctrl_custom("user32.dll", "SendMessageA", handler=handler_SendMessageA),
    api_ctrl_custom("user32.dll", "SendMessageW", handler=handler_SendMessageW),
    api_ctrl_params("user32.dll", "RegisterServicesProcess", [param_log_ctrl(4, "pid", V_PARAM_LOG_INT)]),

    api_ctrl_params("user32.dll", "SetProcessWindowStation"),
    api_ctrl_params("user32.dll", "OpenDesktopA", [param_log_ctrl(8, "desktop", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("user32.dll", "OpenDesktopW", [param_log_ctrl(8, "desktop", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("user32.dll", "SetThreadDesktop"),
    api_ctrl_params("user32.dll", "OpenWindowStationA", [param_log_ctrl(8, "station", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("user32.dll", "OpenWindowStationW", [param_log_ctrl(8, "station", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("user32.dll", "EmptyClipboard"),
    api_ctrl_params("user32.dll", "GetClipboardData"),
    api_ctrl_params("user32.dll", "OpenClipboard"),
    api_ctrl_params("user32.dll", "SetClipboardData"),
    api_ctrl_params("user32.dll", "GetKeyboardState"),
    api_ctrl_params("user32.dll", "SetKeyboardState"),
    api_ctrl_params("user32.dll", "GetAsyncKeyState"),
    api_ctrl_params("user32.dll", "GetKeyState"),
    api_ctrl_params("user32.dll", "keybd_event"),
    api_ctrl_params("user32.dll", "mouse_event"),
    api_ctrl_params("user32.dll", "GetCursorPos"),
    api_ctrl_params("user32.dll", "GetWindowRect"),
    api_ctrl_params("user32.dll", "ScreenToClient"),
    api_ctrl_params("user32.dll", "ClientToScreen"),
    api_ctrl_params("user32.dll", "GetForegroundWindow"),
]

func_win_gdi32 = [
    api_ctrl_params("gdi32.dll", "CreateCompatibleDC"),
    api_ctrl_params("gdi32.dll", "CreateCompatibleBitmap"),
    api_ctrl_params("gdi32.dll", "BitBlt"),
]

func_mutex_kernel32 = [
    api_ctrl_params("kernel32.dll", "ReleaseMutex"),
    api_ctrl_custom("kernel32.dll", "CreateMutexW", handler=handler_CreateMutexW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "OpenMutexW", handler=handler_OpenMutexW, cstk_filter_depth=2),
]

func_evtlog_advapi32 = [
    api_ctrl_params("advapi32.dll", "OpenEventLogA", [param_log_ctrl(4, "svr", V_PARAM_LOG_PASTR), param_log_ctrl(8, "src", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("advapi32.dll", "OpenEventLogW", [param_log_ctrl(4, "svr", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "src", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("advapi32.dll", "ClearEventLogW", [param_log_ctrl(8, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
]

func_priviledge_advapi32 = [
    api_ctrl_params("advapi32.dll", "AdjustTokenPrivileges"),
    api_ctrl_params("advapi32.dll", "LookupPrivilegeDisplayNameW", [param_log_ctrl(4, "sys", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "name", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "LookupPrivilegeNameW", [param_log_ctrl(4, "sys", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "LookupPrivilegeValueW", [param_log_ctrl(4, "sys", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "name", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
]

func_res_kernel32 = [
    api_ctrl_custom("kernel32.dll", "FindResourceA", handler=handler_FindResourceA),
    api_ctrl_custom("kernel32.dll", "FindResourceW", handler=handler_FindResourceW),
    api_ctrl_custom("kernel32.dll", "FindResourceExA", handler=handler_FindResourceExA),
    api_ctrl_custom("kernel32.dll", "FindResourceExW", handler=handler_FindResourceExW),
    api_ctrl_params("kernel32.dll", "LoadResource"),
    api_ctrl_params("kernel32.dll", "LockResource"),
    api_ctrl_params("kernel32.dll", "SizeofResource"),
]

func_res_kernel32_1 = [
    api_ctrl_params("kernel32.dll", "UpdateResourceW", [param_log_ctrl(8, "res_type", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "res_name", V_PARAM_LOG_PUSTR)]),
]

func_pipe_kernel32 = [
    api_ctrl_params("kernel32.dll", "CreateNamedPipeW", [param_log_ctrl(4, "pipe", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "CreatePipe"),
]

func_pipe_kernel32_1 = [
    api_ctrl_params("kernel32.dll", "CallNamedPipeW", [param_log_ctrl(4, "pipe", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "WaitNamedPipeW", [param_log_ctrl(4, "pipe", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "PeekNamedPipe"),
    api_ctrl_params("kernel32.dll", "ConnectNamedPipe"),
    api_ctrl_params("kernel32.dll", "DisconnectNamedPipe"),
]

func_hook_user32 = [
    api_ctrl_params("user32.dll", "SetWindowsHookA", [param_log_ctrl(4, "id", V_PARAM_LOG_INT), param_log_ctrl(8, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "SetWindowsHookW", [param_log_ctrl(4, "id", V_PARAM_LOG_INT), param_log_ctrl(8, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "SetWindowsHookExA", [param_log_ctrl(4, "id", V_PARAM_LOG_INT), param_log_ctrl(8, "cbk", V_PARAM_LOG_INT), param_log_ctrl(0x10, "tid", V_PARAM_LOG_INT)], cstk_filter_depth=2),
    api_ctrl_params("user32.dll", "SetWindowsHookExW", [param_log_ctrl(4, "id", V_PARAM_LOG_INT), param_log_ctrl(8, "cbk", V_PARAM_LOG_INT), param_log_ctrl(0x10, "tid", V_PARAM_LOG_INT)], cstk_filter_depth=2),
]

func_hook_user32_1 = [
    api_ctrl_params("user32.dll", "CallNextHookEx", [param_log_ctrl(8, "code", V_PARAM_LOG_INT)]),
    api_ctrl_params("user32.dll", "UnhookWindowsHook", [param_log_ctrl(4, "code", V_PARAM_LOG_INT)]),
]

func_environment_kernel32 = [
    api_ctrl_params("kernel32.dll", "GetEnvironmentStringsA"),
    api_ctrl_params("kernel32.dll", "GetEnvironmentStringsW"),
    api_ctrl_params("kernel32.dll", "GetEnvironmentVariableA", [param_log_ctrl(4, "name", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "GetEnvironmentVariableW", [param_log_ctrl(4, "name", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("kernel32.dll", "SetEnvironmentVariableA", [param_log_ctrl(4, "name", V_PARAM_LOG_PASTR), param_log_ctrl(8, "value", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "SetEnvironmentVariableW", [param_log_ctrl(4, "name", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "value", V_PARAM_LOG_PUSTR)]),
    api_ctrl_custom("kernel32.dll", "ExpandEnvironmentStringsA", handler=handler_ExpandEnvironmentStringsA),
    api_ctrl_custom("kernel32.dll", "ExpandEnvironmentStringsW", handler=handler_ExpandEnvironmentStringsW),
]

func_profile_kernel32 = [
    api_ctrl_custom("kernel32.dll", "GetPrivateProfileStringA", handler=handler_GetPrivateProfileStringA, cstk_filter_depth=3),
    api_ctrl_custom("kernel32.dll", "GetPrivateProfileStringW", handler=handler_GetPrivateProfileStringW, cstk_filter_depth=3),
    api_ctrl_params("kernel32.dll", "GetPrivateProfileSectionA", [param_log_ctrl(4, "app_name", V_PARAM_LOG_PASTR), param_log_ctrl(0x10, "file", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "GetPrivateProfileSectionW", [param_log_ctrl(4, "app_name", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "WritePrivateProfileSectionA", [param_log_ctrl(4, "app_name", V_PARAM_LOG_PASTR), param_log_ctrl(8, "value", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "file", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "WritePrivateProfileSectionW", [param_log_ctrl(4, "app_name", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "value", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "WritePrivateProfileStringA", [param_log_ctrl(4, "app_name", V_PARAM_LOG_PASTR), param_log_ctrl(8, "key", V_PARAM_LOG_PASTR), param_log_ctrl(0xC, "value", V_PARAM_LOG_PASTR), param_log_ctrl(0x10, "file", V_PARAM_LOG_PASTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "WritePrivateProfileStringW", [param_log_ctrl(4, "app_name", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "key", V_PARAM_LOG_PUSTR), param_log_ctrl(0xC, "value", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
]

func_event_kernel32 = [
    api_ctrl_params("kernel32.dll", "OpenEventW", [param_log_ctrl(0xC, "evt", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "SetEvent"),
    api_ctrl_params("kernel32.dll", "ResetEvent"),
    api_ctrl_params("kernel32.dll", "PulseEvent"),
]

func_timer_user32 = [
    api_ctrl_custom("user32.dll", "SetTimer", handler=handler_SetTimer),
    api_ctrl_params("user32.dll", "SetSystemTimer"),
    api_ctrl_custom("user32.dll", "KillTimer", handler=handler_KillTimer),
    api_ctrl_params("user32.dll", "KillSystemTimer"),
]

func_atom_kernel32 = [
    api_ctrl_params("kernel32.dll", "InitAtomTable", [param_log_ctrl(4, "size", V_PARAM_LOG_INT)]),
    api_ctrl_params("kernel32.dll", "AddAtomA", [param_log_ctrl(4, "str", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "AddAtomW", [param_log_ctrl(4, "str", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("kernel32.dll", "DeleteAtom"),
    api_ctrl_params("kernel32.dll", "FindAtomA", [param_log_ctrl(4, "str", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "FindAtomW", [param_log_ctrl(4, "str", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("kernel32.dll", "GetAtomNameA"),
    api_ctrl_params("kernel32.dll", "GetAtomNameW"),
    api_ctrl_params("kernel32.dll", "GlobalAddAtomA", [param_log_ctrl(4, "str", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "GlobalAddAtomW", [param_log_ctrl(4, "str", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("kernel32.dll", "GlobalDeleteAtom"),
    api_ctrl_params("kernel32.dll", "GlobalFindAtomA", [param_log_ctrl(4, "str", V_PARAM_LOG_PASTR)]),
    api_ctrl_params("kernel32.dll", "GlobalFindAtomW", [param_log_ctrl(4, "str", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("kernel32.dll", "GlobalGetAtomNameA"),
    api_ctrl_params("kernel32.dll", "GlobalGetAtomNameW"),
]

func_com_ole32 = [
    api_ctrl_params("ole32.dll", "CoInitializeEx"),
]

func_com_ole32_1 = [
    api_ctrl_params("ole32.dll", "CoCreateInstanceEx"),
    api_ctrl_params("ole32.dll", "CoUninitialize"),
]

func_mm_kernel32 = [
    api_ctrl_custom("kernel32.dll", "VirtualAllocEx", handler=handler_VirtualAllocEx, cstk_filter_depth=2),
]

func_mm_kernel32_1 = [
    # bp at this api will result in unknown results...
    # api_ctrl_params("kernel32.dll", "TlsAlloc"),
    api_ctrl_custom("kernel32.dll", "VirtualProtectEx", handler=handler_VirtualProtectEx, cstk_filter_depth=2),
]

func_mm_ntdll = [
    api_ctrl_params("ntdll.dll", "RtlReAllocateHeap", [param_log_ctrl(0x10, "size", V_PARAM_LOG_INT)]),
]

func_dotnet_mscoree = [
    # manual load .NET module
    api_ctrl_params("mscoree.dll", "CLRCreateInstance", [param_log_ctrl(4, "name", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("mscoree.dll", "CorBindToRuntimeEx", [param_log_ctrl(4, "version", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "flavor", V_PARAM_LOG_PUSTR)]),
    api_ctrl_params("mscoree.dll", "CorExitProcess"),
]

func_other_dbghelp = [
    api_ctrl_custom("dbghelp.dll", "MiniDumpWriteDump", handler=handler_MiniDumpWriteDump),
    api_ctrl_params("dbghelp.dll", "StackWalk64"),
    api_ctrl_params("dbghelp.dll", "SymFunctionTableAccess64"),
    api_ctrl_params("dbghelp.dll", "SymGetModuleBase64"),
]

func_other_kernel32 = [
    api_ctrl_custom("kernel32.dll", "GetProcAddress", handler=handler_GetProcAddress),
    api_ctrl_params("kernel32.dll", "LoadLibraryExW", [param_log_ctrl(4, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=3),
    api_ctrl_params("kernel32.dll", "IsDebuggerPresent"),

    api_ctrl_custom("kernel32.dll", "SetErrorMode", handler=handler_SetErrorMode),
    api_ctrl_params("kernel32.dll", "SetUnhandledExceptionFilter", [param_log_ctrl(4, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_custom("kernel32.dll", "GetComputerNameW", handler=handler_GetComputerNameW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "GetComputerNameExW", handler=handler_GetComputerNameExW, cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "SetComputerNameW", [param_log_ctrl(4, "name", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "SetComputerNameExW", [param_log_ctrl(8, "name", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "GetCurrentDirectoryA", handler=handler_GetCurrentDirectoryA),
    api_ctrl_custom("kernel32.dll", "GetCurrentDirectoryW", handler=handler_GetCurrentDirectoryW),
    api_ctrl_custom("kernel32.dll", "GetModuleFileNameW", handler=handler_GetModuleFileNameW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "GetVersion", handler=handler_GetVersion),
    api_ctrl_custom("kernel32.dll", "GetVersionExW", handler=handler_GetVersionExW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "GetCommandLineA", handler=handler_GetCommandLineA),
    api_ctrl_custom("kernel32.dll", "GetCommandLineW", handler=handler_GetCommandLineW),
    api_ctrl_params("kernel32.dll", "GetStartupInfoA"),
    api_ctrl_params("kernel32.dll", "GetStartupInfoW"),
    api_ctrl_params("kernel32.dll", "OutputDebugStringA", [param_log_ctrl(4, "str", V_PARAM_LOG_PASTR)], cstk_filter_depth=2, max_invoke_cnt_runtime=100),

    api_ctrl_custom("kernel32.dll", "GetTickCount", handler=handler_GetTickCount, max_invoke_cnt_runtime=50),
    api_ctrl_custom("kernel32.dll", "GetSystemTime", handler=handler_GetSystemTime),
]

func_other_kernel32_1 = [
    api_ctrl_params("kernel32.dll", "QueueUserAPC", [param_log_ctrl(4, "cbk", V_PARAM_LOG_INT)]),
    api_ctrl_params("kernel32.dll", "CreateMailslotW", [param_log_ctrl(4, "name", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "SetSystemPowerState", [param_log_ctrl(4, "is_suspend", V_PARAM_LOG_INT), param_log_ctrl(8, "is_force", V_PARAM_LOG_INT)]),
    api_ctrl_params("kernel32.dll", "SetSystemTime"),
    api_ctrl_params("kernel32.dll", "SetSystemTimeAdjustment"),
    api_ctrl_custom("kernel32.dll", "GetModuleHandleW", handler=handler_GetModuleHandleW, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "GetModuleHandleExW", handler=handler_GetModuleHandleExW, cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "DisableThreadLibraryCalls"),
    api_ctrl_params("kernel32.dll", "FileTimeToSystemTime"),
    api_ctrl_params("kernel32.dll", "SystemTimeToFileTime"),
    api_ctrl_params("kernel32.dll", "SetProcessDEPPolicy"),

    api_ctrl_params("kernel32.dll", "CreateIoCompletionPort"),
    api_ctrl_params("kernel32.dll", "BindIoCompletionCallback"),
    api_ctrl_params("kernel32.dll", "PostQueuedCompletionStatus"),
    api_ctrl_params("kernel32.dll", "GetQueuedCompletionStatus"),

    api_ctrl_custom("kernel32.dll", "IsProcessorFeaturePresent", handler=handler_IsProcessorFeaturePresent),
    api_ctrl_params("kernel32.dll", "CreateHardLinkW", [param_log_ctrl(4, "file_new", V_PARAM_LOG_PUSTR), param_log_ctrl(8, "old_file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "GetLogicalProcessorInformation"),
    api_ctrl_params("kernel32.dll", "GetNativeSystemInfo"),
]

func_other_kernel32_2 = [
    api_ctrl_custom("kernel32.dll", "WaitForSingleObjectEx", handler=handler_WaitForSingleObjectEx, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "WaitForMultipleObjectsEx", handler=handler_WaitForMultipleObjectsEx, cstk_filter_depth=2),
    api_ctrl_params("kernel32.dll", "CreateEventW", [param_log_ctrl(0x10, "evt", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),

    # cause sample D0B78005C40EFC219CC1299A57B85C03 EXCEPTION
    api_ctrl_params("kernel32.dll", "RaiseException"),
]

func_other_mpr = [
    api_ctrl_params("mpr.dll", "WNetUseConnectionW", [param_log_ctrl(0xC, "pwd", V_PARAM_LOG_PUSTR), param_log_ctrl(0x10, "userid", V_PARAM_LOG_PUSTR)], cstk_filter_depth=3),
]

func_other_psapi = [
    api_ctrl_custom("psapi.dll", "GetModuleFileNameExW", handler=handler_GetModuleFileNameExW, cstk_filter_depth=2),
    api_ctrl_custom("psapi.dll", "GetProcessImageFileNameA", handler=handler_GetProcessImageFileNameA),
    api_ctrl_custom("psapi.dll", "GetProcessImageFileNameW", handler=handler_GetProcessImageFileNameW),
]

func_other_msvcrt = [
    api_ctrl_custom("msvcrt.dll", "_access", handler=handler_access),
]

func_other_msi = [
    api_ctrl_params("msi.dll", "MsiViewFetch"),
]

func_other_shell32 = [
    api_ctrl_custom("shell32.dll", "ShellExecuteExW", handler=handler_ShellExecuteExW, cstk_filter_depth=3),
]

func_other_shell32_1 = [
    api_ctrl_custom("shell32.dll", "SHGetFolderPathW", handler=handler_SHGetFolderPathW, cstk_filter_depth=3),
]

func_other_advapi32 = [
    api_ctrl_params("advapi32.dll", "OpenProcessToken"),
    api_ctrl_params("advapi32.dll", "EncryptFileW", [param_log_ctrl(4, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
    api_ctrl_params("advapi32.dll", "DecryptFileW", [param_log_ctrl(4, "file", V_PARAM_LOG_PUSTR)], cstk_filter_depth=2),
]

func_other_ntdll = [
    api_ctrl_custom("ntdll.dll", "ZwDelayExecution", handler=handler_ZwDelayExecution),
    # api_ctrl_custom("ntdll.dll", "NtQueryInformationProcess", handler=handler_NtQueryInformationProcess),
    # api_ctrl_custom("ntdll.dll", "NtQueryInformationThread", handler=handler_NtQueryInformationThread),
    api_ctrl_custom("ntdll.dll", "NtSetInformationProcess", handler=handler_NtSetInformationProcess),
]
...
func_xx = [
    api_ctrl_custom("kernel32.dll", "SleepEx", handler=handler_SleepEx, cstk_filter_depth=2),
    api_ctrl_custom("kernel32.dll", "TerminateProcess", handler=handler_TerminateProcess),
    api_ctrl_custom("kernel32.dll", "ExitProcess", handler=handler_ExitProcess),
]

func_test = [
    api_ctrl_custom("msvbvm60.dll", "DllFunctionCall", handler=handler_DllFunctionCall),
]
...
def gather_alloc_apis():
    ...
    alloc_func = [
        api_ctrl_custom("kernel32.dll", "VirtualAllocEx", handler=handler_VirtualAllocEx, cstk_filter_depth=2),
        # api_ctrl_params("kernel32.dll", "TlsAlloc"),
        api_ctrl_custom("ntdll.dll", "RtlAllocateHeap", handler=handler_RtlAllocateHeap, cstk_filter_depth=2),
        api_ctrl_custom("ntdll.dll", "RtlReAllocateHeap", handler=handler_RtlReAllocateHeap, cstk_filter_depth=2),
    ]
...
def handler_all_api_proxy(dbg):
    ...
    v_tmp_last_api_name[dbg.dbg.dwThreadId] = api_ctrl.api_name

    # ---------------------------------------------------------------------------
    # api invoke count

    global v_tmp_api_invoke_summary
    if api_ctrl.api_name not in v_tmp_api_invoke_summary:
        v_tmp_api_invoke_summary[api_ctrl.api_name] = (0, 0, 1)
    else:
        ok_cnt = v_tmp_api_invoke_summary[api_ctrl.api_name][0]
        no_cnt = v_tmp_api_invoke_summary[api_ctrl.api_name][1]
        all_cnt = v_tmp_api_invoke_summary[api_ctrl.api_name][2]
        v_tmp_api_invoke_summary[api_ctrl.api_name] = (ok_cnt, no_cnt, all_cnt + 1)
    ...
    if v_tmp_is_check_call_stack:
        ...
        is_ok = False
        cstks_tmp = dbg.get_call_stacks_default(depth=api_ctrl.cstk_filter_depth, is_fix_api_start=True)
        ...
        for from_to_pair in ignore_from_to_pairs:
            if api_ctrl.api_name == from_to_pair[0] and len(cstks_tmp) >= 2:
    ...
    all_cnt = v_tmp_api_invoke_summary[api_ctrl.api_name][2]
    if is_ok:
        v_tmp_api_invoke_summary[api_ctrl.api_name] = (ok_cnt + 1, no_cnt, all_cnt)
    else:
        v_tmp_api_invoke_summary[api_ctrl.api_name] = (ok_cnt, no_cnt + 1, all_cnt)
    ...
    if is_ok:
        ...
        param_dict = {}
        if isinstance(api_ctrl, api_ctrl_params):

            # not-custom, might only log params
            if api_ctrl.param_log_ctrl_list is None or len(api_ctrl.param_log_ctrl_list) == 0:

                # no params needed
                _xrk_api_invoke_detail(dbg, api_ctrl.api_name)

            else:
                # get params
                for param_log_ctrl in api_ctrl.param_log_ctrl_list:

                    # _pt_log(">>> get log %s - %s" % (api_ctrl.api_name, param_log_ctrl))

                    param_dict[param_log_ctrl.log_name] = param_log_ctrl.get_log(dbg)
                _xrk_api_invoke_detail(dbg, api_ctrl.api_name, param_dict)

        else:
            # proxy to custom handler
            param_dict = api_ctrl.handler(dbg)
            # looking for bugs....
            if param_dict is None:
                _pt_log(">>> custom handler of api %s retn None" % api_ctrl.api_name)
                param_dict = {}

        # maybe del bp if api invoke too frequently
        global v_tmp_api_invoke_cnt_dict_runtime
        if api_ctrl.api_name not in v_tmp_api_invoke_cnt_dict_runtime:
            v_tmp_api_invoke_cnt_dict_runtime[api_ctrl.api_name] = 1
        else:
            v_tmp_api_invoke_cnt_dict_runtime[api_ctrl.api_name] = v_tmp_api_invoke_cnt_dict_runtime[api_ctrl.api_name] + 1
            # may del bp
            if api_ctrl.max_invoke_cnt_runtime is not None and api_ctrl.max_invoke_cnt_runtime < v_tmp_api_invoke_cnt_dict_runtime[api_ctrl.api_name]:
                dbg.bp_del(dbg.context.Eip)
                # _pt_log(">>> api invoke reach max limit, remove bp: %d - %s" % (api_ctrl.max_invoke_cnt_runtime, api_ctrl.api_name))
        ...
        output.add_api_call_record(api_ctrl.api_name, get_resolved_call_stacks_default(dbg, is_fix_api_start=True), params_str)

    else:
        # _pt_log("no - %s" % api_ctrl.api_name)
        pass
    ...
    if v_tmp_is_pt_api_invoke_time:
        ...
        _pt_log("api invoke: %s, time: %.8d msecs" % (api_ctrl.api_name, (end - start).microseconds / 1000))
...
def dbg_set_api_bp(dbg, api_ctrl):
    ...
    """
    addr = dbg.func_resolve(api_ctrl.dll_name, api_ctrl.api_name)
