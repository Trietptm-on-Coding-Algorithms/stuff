...
class pydbg:
    ...
    def bp_del(self, address):
        ...
        @param: address : int : Address or list of addresses to remove breakpoint from
    ...
    def bp_del_hw(self, address=None, slot=None):
        ...
        @param: address : int : (Optional) Address to remove hardware breakpoint from.
        @param: slot    : int : (Optional)(0 through 3)

        @return: self :
        @raise pdx: An exception is raised on failure.
        """
        if address == slot and slot is None:
    ...
    def bp_del_mem(self, address):
        ...
        @param: address : int : Address or list of addresses to remove memory breakpoint from

        @return: self :
        @raise pdx: An exception is raised on failure.
        """

        self._log("bp_del_mem(%08X )" % address)
    ...
    def bp_is_ours_mem(self, address_to_check):
        ...
        When handling potential memory breakpoint exceptions it is mandatory to check the offending address with this routine,
        as memory breakpoints are implemented by changing page permissions and the referenced address may very well exist
    ...
    def disasm(self, address):
        ...
        @param: address : int : Address to disassemble at
    ...
    def debug_event_iteration(self):
        ...
        if kernel32.WaitForDebugEvent(byref(dbg), 100) != 0:
            ...
            # an exception was caught.
                ...
                self._log(">>> evt(%.8X): exception: %.8X" % (self.dbg.dwThreadId, ec))
                ...
                # unhandled exception.
                    ...
                    if dbg.u.Exception.dwFirstChance:
                        self._warn("[1st Chance] TID:%04x caused an unhandled exception (%08X - %s) at %08X " % (self.dbg.dwThreadId, ec, msdn.resolve_code_exception(ec), self.exception_address))
                    else:
                        self._warn("[2nd Chance] TID:%04x caused an unhandled exception (%08X - %s) at %08X " % (self.dbg.dwThreadId, ec, msdn.resolve_code_exception(ec), self.exception_address))

                    continue_status = DBG_EXCEPTION_NOT_HANDLED

            # OUTPUT_DEBUG_STRING_EVENT 8
            elif dbg.dwDebugEventCode == 8:
                if self.dbg.u.DebugString.fUnicode == 0:
                    dbg_str = "ansi...."
                else:
                    dbg_str = "unicode...."
                self._log("evt(%.8X): dbg string: %s" % (self.dbg.dwThreadId, dbg_str))

                continue_status = DBG_EXCEPTION_NOT_HANDLED

            # RIP_EVENT 9
            elif dbg.dwDebugEventCode == 9:
                self._log("evt(%.8X): rip, error: %d, type: %d" % (self.dbg.dwThreadId, self.dbg.u.RipInfo.dwError, self.dbg.u.RipInfo.dwType))

                continue_status = DBG_CONTINUE

            else:
                self._high(">>> evt(%.8X): invalid" % (self.dbg.dwThreadId))
                continue_status = DBG_EXCEPTION_NOT_HANDLED
        ...
        if kernel32.ContinueDebugEvent(dbg.dwProcessId, dbg.dwThreadId, continue_status) == 0:
            error = kernel32.GetLastError()
            if error == 87:
    ...
    def event_handler_load_dll(self):
        ...
        self._log(">>> evt(%.8X): load dll: %s" % (self.dbg.dwThreadId, dll.name))
    ...
    def event_handler_unload_dll(self):
        ...
        for system_dll in self.system_dlls:
            if system_dll.base == base:
                ...
                self._log(">>> evt(%.8X): unload dll: %s" % (self.dbg.dwThreadId, unloading.name))
                break
    ...
    def retrive_unicode_string(self, data, max_len=1024):
        """
        description
        ...
        @return: string : string, not unicode string
               : False  : failure
        """
        ret = util.data_to_unicode_str(data, max_len)
        if len(ret) < 2:
            return False
        ...
        try:
            ...
            self._err("convert unicode to string fail...")
            return False
    ...
    def read_process_memory(self, address, length):
        ...
        @param: address             : int  : Address to read from.
    ...
    def read_int8(self, address):
        buf = self.read_process_memory(address, 1)

        if len(buf) != 1:
            # todo: we shall fix this
            self._err("read int8 from addr %.8X, but got but len: %d" % (address, len(buf)))
            return 0

        return ord(buf)

    def read_int16(self, address):
        buf = self.read_process_memory(address, 2)

        if len(buf) != 2:
            # todo: we shall fix this
            self._err("read int16 from addr %.8X, but got but len: %d" % (address, len(buf)))
            return 0
    ...
    def read_int32(self, address):
        buf = self.read_process_memory(address, 4)

        if len(buf) != 4:
            # todo: we shall fix this
            self._err("read int32 from addr %.8X, but got but len: %d" % (address, len(buf)))
            return 0

        v = struct.unpack('<L', buf)[0]
        return c_uint32(v).value

    def read_stack_int8(self, offset):
        return self.read_int8(self.context.Esp + offset)

    def read_stack_int16(self, offset):
        return self.read_int16(self.context.Esp + offset)

    def read_stack_int32(self, offset):
        return self.read_int32(self.context.Esp + offset)

    def read_stack_p_int8(self, offset):
        address = self.read_int32(self.get_register("ESP") + offset)
        return self.read_int8(address)

    def read_stack_p_int16(self, offset):
        address = self.read_int32(self.get_register("ESP") + offset)
        return self.read_int16(address)

    def read_stack_p_int32(self, offset):
        address = self.read_int32(self.get_register("ESP") + offset)
    ...
    def read_ascii_string(self, address, max_bytes=1024):
        """
        ...
        """
        buf = self.read_process_memory(address, 1)
        if buf is not None and len(buf) == 1 and buf[0] != '\00':
            ...
            ret = self.retrive_ascii_string(self.read_process_memory(address, max_bytes))
            if not ret and max_bytes >= 1024:

                ret = self.retrive_ascii_string(self.read_process_memory(address, 256))
                if not ret:
                    ret = self.retrive_ascii_string(self.read_process_memory(address, 48))

            return ret and ret or None

        return None

    def read_printable_string(self, address, max_bytes=1024):
        """
        ...
        """
        buf = self.read_process_memory(address, 1)
        if buf is not None and len(buf) == 1 and self.is_char_printable(buf[0]):

            # this is too ugly
            # if required bytes is too "large", this "read" action might return None, so we need to minimize size required, and read many times
            ret = self.retrive_printable_string(self.read_process_memory(address, max_bytes))
            if not ret and max_bytes >= 1024:

                ret = self.retrive_printable_string(self.read_process_memory(address, 256))
                if not ret:
                    ret = self.retrive_printable_string(self.read_process_memory(address, 48))

            return ret and ret or None

        return None

    def read_unicode_string(self, address, max_bytes=1024):
        """
        ...
        """
        buf = self.read_process_memory(address, 2)
        if buf is not None and len(buf) == 2 and (buf[0] != '\00' or buf[1] != '\00'):

            # this is too ugly
            # if required bytes is too "large", this "read" action might return None, so we need to minimize size required, and read many times
            ret = self.retrive_unicode_string(self.read_process_memory(address, max_bytes))
            if not ret and max_bytes >= 1024:

                ret = self.retrive_unicode_string(self.read_process_memory(address, 256))
                if not ret:
                    ret = self.retrive_unicode_string(self.read_process_memory(address, 48))

            return ret and ret or None

        return None

    def read_p_ascii_string(self, address, max_bytes=1024):
        """
            ...
            @return: string :
                   : None   :
        """
        address = self.read_int32(address)
        return self.read_ascii_string(address, max_bytes)

    def read_p_printable_string(self, address, max_bytes=1024):
        """
            @param: address   : int : address to read
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
        ...
        """
        address = self.read_int32(address)
        return self.read_printable_string(address, max_bytes)

    def read_p_unicode_string(self, address, max_bytes=1024):
        """
            @param: address   : int : address to read
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
        ...
        """
        address = self.read_int32(address)
        return self.read_unicode_string(address, max_bytes)

    def read_pp_ascii_string(self, address, max_bytes=1024):
        """
            @param: address   : int : address to read
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
    ...
    def read_pp_printable_string(self, address, max_bytes=1024):
        """
            @param: address   : int : address to read
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
    ...
    def read_pp_unicode_string(self, address, max_bytes=1024):
        """
            @param: address   : int : address to read
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
    ...
    def read_stack_ascii_string(self, offset, max_bytes=1024):
        """
            ...
            @return: string :
                   : None   :
        """
        return self.read_ascii_string(self.get_register("ESP") + offset, max_bytes)

    def read_stack_printable_string(self, offset, max_bytes=1024):
        """
            @param: offset    : int : stack offset
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
            ...
            @return: string :
                   : None   :
        """
        return self.read_printable_string(self.get_register("ESP") + offset, max_bytes)

    def read_stack_unicode_string(self, offset, max_bytes=1024):
        """
            @param: offset    : int : stack offset
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
            ...
            @return: string : !+ string, not unicode string
                   : None   :
        """
        return self.read_unicode_string(self.get_register("ESP") + offset, max_bytes)

    def read_stack_p_ascii_string(self, offset, max_bytes=1024):
        """
            @param: offset    : int : stack offset
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
    ...
    def read_stack_p_printable_string(self, offset, max_bytes=1024):
        """
            @param: offset    : int : stack offset
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
    ...
    def read_stack_p_unicode_string(self, offset, max_bytes=1024):
        """
            @param: offset    : int : stack offset
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
    ...
    def read_stack_pp_ascii_string(self, offset, max_bytes=1024):
        """
            @param: offset    : int : stack offset
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
    ...
    def read_stack_pp_printable_string(self, offset, max_bytes=1024):
        """
            @param: offset    : int : stack offset
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
    ...
    def read_stack_pp_unicode_string(self, offset, max_bytes=1024):
        """
            @param: offset    : int : stack offset
            @param: max_bytes : int : (optional, dft=1024) max string bytes to read
    ...
    def write_process_memory(self, address, data, length=0):
        ...
        @param: address : int : Address to write to
    ...
    def write_int8(self, address, value):
        assert value >= -0xFF
        assert value <= 0xFF
        self.write(address, int2str8_swap(value), length=1)

    def write_int16(self, address, value):
        assert value >= -0xFFFF
        assert value <= 0xFFFF
        self.write(address, int2str16_swapped(value), length=2)

    def write_int32(self, address, value):
        assert value >= -0xFFFFFFFF
        assert value <= 0xFFFFFFFF
        self.write(address, int2str32_swapped(value), length=4)

    def write_ascii_string(self, address, str_):
    ...
    def write_unicode_string(self, address, str_):
        encoding = chardet.detect(str_)['encoding']
        if encoding == "ascii":
    ...
    def write_stack_int8(self, offset, value):
        self.write_int8(self.context.Esp + offset, value)

    def write_stack_int16(self, offset, value):
        self.write_int16(self.context.Esp + offset, value)

    def write_stack_int32(self, offset, value):
        self.write_int32(self.context.Esp + offset, value)

    def write_stack_ascii_string(self, offset, str_):
        self.write_ascii_string(self.context.Esp + offset, value)

    def write_stack_unicode_string(self, offset, str_):
        self.write_unicode_string(self.context.Esp + offset, value)

    def write_stack_p_int8(self, offset, value):
        addr = self.read_int32(self.context.Esp + offset)
        self.write_int8(addr, value)

    def write_stack_p_int16(self, offset, value):
        addr = self.read_int32(self.context.Esp + offset)
        self.write_int16(addr, value)

    def write_stack_p_int32(self, offset, value):
        addr = self.read_int32(self.context.Esp + offset)
        self.write_int32(addr, value)

    def write_stack_p_ascii_string(self, offset, str_):
        addr = self.read_int32(self.context.Esp + offset)
        self.write_ascii_string(addr, str_)

    def write_stack_p_unicode_string(self, offset, str_):
        addr = self.read_int32(self.context.Esp + offset)
        self.write_unicode_string(addr, str_)

    def write_stack_pp_int8(self, offset, value):
        addr = self.read_int32(self.context.Esp + offset)
        addr = self.read_int32(addr)
        self.write_int8(addr, value)

    def write_stack_pp_int16(self, offset, value):
        addr = self.read_int32(self.context.Esp + offset)
        addr = self.read_int32(addr)
        self.write_int16(addr, value)

    def write_stack_pp_int32(self, offset, value):
        addr = self.read_int32(self.context.Esp + offset)
        addr = self.read_int32(addr)
        self.write_int32(addr, value)

    def write_stack_pp_ascii_string(self, offset, str_):
        addr = self.read_int32(self.context.Esp + offset)
        addr = self.read_int32(addr)
        self.write_ascii_string(addr, str_)

    def write_stack_pp_unicode_string(self, offset, str_):
        addr = self.read_int32(self.context.Esp + offset)
        addr = self.read_int32(addr)
        self.write_unicode_string(addr, str_)
    ...
    def seh_unwind(self, context=None):
        ...
        In cases where the SEH chain is corrupted and the handler address points to invalid memory, it will be returned as 0xFFFFFFFF.
    ...
    def smart_dereference(self, address, print_dots=True, hex_dump=False):
        ...
        @param: address    : int  : Address to smart dereference
        @param: print_dots : bool : (Optional, def:True) Controls suppression of dot in place of non-printable
    ...
    def virtual_alloc(self, address, size, alloc_type, protection):
        ...
        @param: address    : int : Desired starting address of region to allocate, can be None
        ...
        @return: int : Base address of the allocated region of pages.
        @raise pdx: An exception is raised on failure.
        """
    ...
    def virtual_free(self, address, size, free_type):
        ...
        @param: address   : int : Pointer to the starting address of the region of memory to be freed
    ...
    def virtual_protect(self, base_address, size, protection):
        ...
        @param: base_address : int : Base address of region of pages whose access protection attributes are to be changed
    ...
    def virtual_query(self, address):
        ...
        @param: address : int : Address to query
    ...
    def read_msr(self, address):
        ...
        @param: address : int : MSR address to read from.
    ...
    def write_msr(self, address, data):
        ...
        @param: address : int : MSR address to write to.
        @param: data    : int : Data to write to MSR address.
    ...
    def convert_fromto_to_call_stack(self, from_addr, to_addr):
        """
            convert call stack from_addr and to_addr to _share_this.call_stack() object

            @param: from_addr : int : call from address
            @param: to_addr   : int : return to address

            @return: obj : _share_this.call_stack() object
        """
        assert from_addr != 0
        from_md = self.addr_to_module(from_addr)
        if from_md is not None:
            from_md_name = from_md.szModule.lower()
            from_md_offset = from_addr - from_md.modBaseAddr
            assert from_md_name is not None and len(from_md_name) != 0
        else:
            from_md_name = None
            from_md_offset = from_addr

        if to_addr != 0:
            to_md = self.addr_to_module(to_addr)
            if to_md is not None:
                to_md_name = to_md.szModule.lower()
                to_md_offset = to_addr - to_md.modBaseAddr
                assert to_md_name is not None and len(to_md_name) != 0
            else:
                to_md_name = None
                to_md_offset = to_addr
        else:
            to_md_name = None
            to_md_offset = 0

        return _share_this.call_stack("", from_addr, from_md_name, from_md_offset, to_addr, to_md_name, to_md_offset)

    def get_call_stack_thread(self, h_thread, ctx_thread, depth=None, is_fix_api_start=False):
        """
            @param: h_thread         :
            @param: ctx_thread       :
            @param: depth            :
            ...
            @return: list : a list of _share_this.call_stack() obj. list maybe empty.
        """
        ...
        # winappdbg.thread.py doesn't have this, which is very incorrect.
        frame.AddrPC.Mode = 3
        frame.AddrFrame.Mode = 3
        frame.AddrStack.Mode = 3

        # self.suspend_thread(self.dbg.dwThreadId)

        if is_fix_api_start:
            fix_to_addr = self.read_stack_int32(0)

        ret = []
        # while windll.dbghelp.StackWalk64(IMAGE_FILE_MACHINE_I386, self.h_process, self.h_thread, byref(frame), byref(ctx_thread_copy), None, None, None, None):
        while windll.dbghelp.StackWalk64(IMAGE_FILE_MACHINE_I386, self.h_process, h_thread, byref(frame), None, None, None, None, None):
            ...
            if frame.AddrPC.Offset != 0:
                ...
                # addr : frame.AddrFrame.Offset + 4 (stack address)
                # from : frame.AddrPC.Offset (function start address)
                # to   : frame.AddrReturn.Offset (return to address)

                # todo: fix_to_addr shall be valid address
                if is_fix_api_start and len(ret) == 0 and fix_to_addr > 0x1000:
                    # split the first frame into 2 frames
                    ret.append(self.convert_fromto_to_call_stack(frame.AddrPC.Offset, fix_to_addr))
                    ret.append(self.convert_fromto_to_call_stack(fix_to_addr, frame.AddrReturn.Offset))

                else:
                    ret.append(self.convert_fromto_to_call_stack(frame.AddrPC.Offset, frame.AddrReturn.Offset))
            else:
                break

            # this 126 is ugly...
            if depth is not None and len(ret) >= depth or len(ret) > 126:
                break

        # self.resume_thread(self.dbg.dwThreadId)

        return ret

    def get_call_stacks_default(self, depth=None, is_fix_api_start=False):
        """
            get call stack of self.h_thread

            @param: depth            : int or None : (optional, dft=None)stack depth. default None, meaning all stacks.
            ...
            ref:
                ...
                winappdbg\thread.py
        """
        return self.get_call_stack_thread(h_thread=self.h_thread, ctx_thread=self.context, depth=depth, is_fix_api_start=is_fix_api_start)

    def get_call_stacks_all(self, depth=None, is_fix_api_start=False):
        """
        ...
        for tid in self.enumerate_threads():
            ...
            ret[tid] = self.get_call_stack_thread(h_thread=h_thread, ctx_thread=ctx_thread, depth=depth, is_fix_api_start=is_fix_api_start)
            self.close_handle(h_thread)
